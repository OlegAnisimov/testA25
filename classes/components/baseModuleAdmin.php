<?php
	...
	use UmiCms\System\Orm\Entity\iFacade as iEntityFacade;
	...
	trait baseModuleAdmin {
		...
		public function prepareDataPage($inputData) {
			...
			foreach ($this->optionList as $name => $value) {
				$dataModule->setOption($name, $value);
			}
		...
		public function prepareDataObject($inputData) {
			...
			if ($this->actionType == 'create') {
				...
				$typeGUID = $typesCollection->getType($typeId)->getGUID();

				$object['attribute:type-id'] = $typeId;
				$object['attribute:type-guid'] = $typeGUID;
		...
		public function saveEditedElementData($inputData) {
			...
			$element->commit();
			Service::StaticCache()->deletePageListCache([$element->getId()]);
		...
		public function saveEditedBaseTypesList($data, $dels) {
			...
			if (is_array($data)) {
				foreach ($data as $id => $info) {
					...
					$hidePages = getArrayKey($info, 'hide-pages') !== null;

					if ($id == 'new') {
						if ($module && $title) {
							$collection->addType($module, $title, $method, $hidePages);
						}
					} else {
		 ...
		 * @param bool|int $typeId идентификатор типа данных поля
		 * @throws coreException
		 * @throws publicAdminException
		 * @throws wrongParamException
		 */
		public function saveEditedFieldData($field, $typeId = false) {
			...
			if ($typeId && $name !== $field->getName()) {
				$type = $this->getObjectTypeById($typeId);
				$this->validateFieldName($type, $name);
			}
		...
		public function saveAddedFieldData($inputData) {
			...
			foreach ($childTypeIds as $childTypeId) {
				...
				if ($childGroup instanceof iUmiFieldsGroup) {
		...
		public function deleteElement($params) {
			...
			umiHierarchy::getInstance()->delElement($element->getId(), false);
		...
		public function setDirectCallError() {
			$this->setData([
				'message' => 'This method returns result only by direct xml or json call'
			]);
		}
		...
		/**
		 * Формирует и возвращает простое результирующее сообщение об успешном окончании действия табличного контрола
		...
		/**
		 * Формирует и возвращает простое результирующее сообщение об ошибочном окончании действия  табличного контрола
		 * @param mixed $message
		 * @return array
		 */
		...
		/**
		 * Формирует и возвращает простое результирующее сообщение для запуска обновления табличного контрола
		 * @param mixed $message
		 * @return array
		 */
		public function getSimpleRefreshMessage($message = true) {
			return [
				'refresh' => $message
			];
		}
		...
		public function getLimit() {
			...
			return $limit ?: $this->getDefaultPerPageNumber();
		}

		/**
		 * Возвращает или определяет ограничение на количество сущностей в табличном контроле
		 * @param int|string|null $limit ограничение на количество сущностей в табличном контроле
		 * @return int|string
		 */
		public function getOrDetectLimit($limit = null) {
			return $limit ?: $this->getLimit();
		}

		/**
		 * Возвращает или определяет значения фильтрации
		 * @param array $filter значения фильтрации
		 * @return array
		 */
		public function getOrDetectFilter(array $filter) : array {
			return $filter ?: $this->getFilterValues();
		}

		/**
		 * Возвращает или определяет значения сортировки
		 * @param array $sort значения сортировки
		 * @return array
		 */
		public function getOrDetectSort(array $sort) : array {
			return $sort ?: $this->getSortValues();
		}
		...
		/**
		 * Возвращает или определяет смещение результат выборки сущностей в табличном контроле
		 * @param int|string|null $limit ограничение на количество сущностей в табличном контроле
		 * @param int|null $offset смещение результат выборки сущностей в табличном контроле
		 * @return int
		 */
		public function getOrDetectOffset($limit, int $offset = null) : int {
			return $offset ?: $this->getOffset($limit);
		}
		...
		/**
		 * Возвращает идентификатор сущности табличного контрола, относительно которой перемещаются другие сущности
		 * @return string|null
		 */
		public function getDragTargetEntityId() {
			$data = $this->getDragTargetEntityData();
		...
		/**
		 * Возвращает список идентификаторов перемещаемых сущностей табличного контрола
		 * @return int[]
		 */
		public function getDraggedEntitiesIdList() {
			return array_map(function($data) {
				return isset($data['id']) ? (int) $data['id'] : null;
			}, $this->getDraggedEntitiesData());
		}
		...
		/**
		 * Возвращает идентификатор домена для табличного контрола
		 * @param int|null $domainId идентификатор домена
		 * @return int
		 * @throws coreException
		 */
		public function getOrDetectDomainId(int $domainId = null) : int {
			return $domainId ?: ($this->getDomainId() ?: Service::DomainDetector()->detectId());
		}
		...
		/**
		 * Возвращает идентификатор языка для табличного контрола
		 * @param int|null $languageId идентификатор языка
		 * @return int
		 * @throws coreException
		 */
		public function getOrDetectLanguageId(int $languageId = null) : int {
			return $languageId ?: ($this->getLanguageId() ?: Service::LanguageDetector()->detectId());
		}
		...
		/**
		 * Возвращает часть списка дочерних страниц
		 */
		public function getChildrenPart() {
			$this->getChildrenList('get');
		}

		/**
		 * Возвращает часть списка дочерних удаленных страниц
		 */
		public function getDeletedChildrenPart() {
			$this->getChildrenList('getDeleted');
		}

		/**
		 * Возвращает часть списка дочерних активных страниц
		 */
		public function getActiveChildrenPart() {
			$this->getChildrenList('getActive');
		}

		/**
		 * Возвращает часть списка дочерних неактивных страниц
		 */
		public function getInactiveChildrenPart() {
			$this->getChildrenList('getInactive');
		}

        /**
         * Возвращает расширение иерархического типа сущности,
         * над которой выполняется операция в административной панели
         * @return bool|string
         * @throws coreException
         */
        public function getCurrentEntityHierarchicalTypeExtension() {
            $extension = false;

            if (isset($this->data['object']['attribute:id'])) {
                $objectId = $this->data['object']['attribute:id'];
                $object = umiObjectsCollection::getInstance()->getObject($objectId);

                if ($object instanceof iUmiObject) {
                    $extension = $this->getHierarchicalTypeExtension($object->getTypeId()) ?: $extension;
                }
            }

            if (!$extension && isset($this->data['page']['attribute:id'])) {
                $elementId = $this->data['page']['attribute:id'];
                $element = umiHierarchy::getInstance()->getElement($elementId);

                if ($element instanceof iUmiHierarchyElement) {
                    $extension = $element->getMethod();
                }
            }

            if (!$extension && isset($this->data['page']['attribute:type-id'])) {
                $objectTypeId = $this->data['page']['attribute:type-id'];
                $extension = $this->getHierarchicalTypeExtension($objectTypeId) ?: $extension;
            }

            if (!$extension && isset($this->data['object']['attribute:type-id'])) {
                $objectTypeId = $this->data['object']['attribute:type-id'];
                $extension = $this->getHierarchicalTypeExtension($objectTypeId) ?: $extension;
            }
		...
		/**
		 * Делает срез списка строк для постраничной навигации нового табличного контрола
		 * @param array[]|iTemplate[]|iUmiFile[] $rowList список шаблонов
		 * @param int|null $limit размер среза
		 * @param int|null $offset смещение среза
		 * @return array
		 */
		public function sliceDataRowList(array $rowList, int $limit = null, int $offset = null) : array {
			$limit = $this->getOrDetectLimit($limit);
			$offset = $this->getOrDetectOffset($limit);
			...
			return array_slice($rowList, $offset, $limit);
		}

		/**
		 * Инициирует скачивание файла
		 * @param iUmiDirectory $directory директория
		 * @param string|null $fileName имя файла
		 */
		public function downloadFile(iUmiDirectory $directory, string $fileName = null) {
			if (isDemoMode()) {
			...
			try {
				$this->getFile($directory, $fileName)
					->download();
			} catch (Exception $exception) {
		 ...
		 */
		public function deleteFileList(iUmiDirectory $directory, array $fileNameList = []) {
			if (isDemoMode()) {
				$result = $this->getSimpleErrorMessage(getLabel('label-stop-in-demo'));
				$this->printEntityTableControlResult($result);
				return;
			...
			try {
				$fileNameList = $fileNameList ?: Service::Request()->Post()->get('fileNameList');
		 ...
		 */
		public function getFile(iUmiDirectory $directory, ?string $fileName) : iUmiFile {
		 ...
		 */
		public function getDirectory(string $path) : iUmiDirectory {
			$directory = Service::DirectoryFactory()
				->create($path);
			$directory::requireFolder($path);
			return $directory;
		}

		/**
		 * Сохраняет настройки Google авторизации
		 ...
		 * @throws privateException
		 */
		public function saveGoogleOAuthConfig(array $config) {
			$this->setRegistryValue('service-account', trim((string) $config['string:google-service-account']));

			if (isset($config['text:google-private-key'])) {
				try {
					$privateKey = trim((string) $config['text:google-private-key']);
					$this->getPrivateKeysBunch()
						->set(iOAuth::PRIVATE_KEY_NAME, $privateKey);
				} catch (ErrorException $exception) {
					/** @var def_module $module */
					$module = $this->module;
					$module->errorNewMessage(getLabel('label-error-google-private-key-save'));
					$module->errorPanic();
				}
			}
		...
		/**
		 * Возвращает настройки Google авторизации
		 * @return array
		 * @throws privateException
		 */
		public function loadGoogleOAuthConfig() : array {
			$privateKey = $this->getGoogleOAuthPrivateKey();
			$type = ($privateKey === null) ? 'text' : 'password';
			return [
				'string:google-service-account' => $this->getRegistryValue('service-account'),
				"$type:google-private-key" => null
			];
		}

		/**
		 * Удаляет приватный ключ для Google авторизации
		 * @throws ErrorException
		 * @throws coreException
		 * @throws errorPanicException
		 * @throws privateException
		 */
		public function deleteGoogleOAuthPrivateKey() {
			try {
				$this->getPrivateKeysBunch()
					->delete(iOAuth::PRIVATE_KEY_NAME);
			} catch (ErrorException $exception) {
				/** @var def_module $module */
				$module = $this->module;
				$module->errorNewMessage(getLabel('label-error-google-private-key-delete'));
				$module->errorPanic();
			}
		...
		/**
		 * Возвращает приватный ключ для Google авторизации
		 * @return string|null
		 * @throws Exception
		 */
		private function getGoogleOAuthPrivateKey() {
			try {
				return $this->getPrivateKeysBunch()
					->get(iOAuth::PRIVATE_KEY_NAME);
			} catch (\ErrorException $exception) {
				return null;
			}
		}
		...
		/**
		 * Возвращает связку приватных ключей
		 * @return iBunch
		 * @throws \Exception
		 */
		private function getPrivateKeysBunch() : iBunch {
			return Service::get(iBunch::SERVICE_NAME);
		}

		/**
		 * Возвращает часть списка дочерних страниц
		 * @param string $method имя метода для получение страниц
		 */
		protected function getChildrenList($method) {
			$parentId = Service::Request()->Get()->get('parentId');
			$childIdList = Service::PageChildrenIdGetter()->$method($parentId);
			$childIdNodeList = $this->prepareChildIdNodeList($childIdList);
			$this->printEntityTableControlResult($childIdNodeList);
		}

		/**
		 * Подготавливает список идентификаторов дочерних страниц к сериализации
		 * @param int[] $childIdList список идентификаторов дочерних страниц
		 * @return array
		 */
		private function prepareChildIdNodeList(array $childIdList) {
			return [
				'nodes:child' => array_map(function ($childId) {
					return [
						'@id' => $childId
					];
				}, $childIdList)
		...
		/**
		 * Возвращает расширение иерархического типа сущности по идентификатору объектного типа
		 * @param int $objectTypeId идентификатор типа объекта
		 * @return string|false
		 * @throws coreException
		 */
		private function getHierarchicalTypeExtension($objectTypeId) {
			...
			return ($hierarchyType instanceof iUmiHierarchyType) ? $hierarchyType->getExt() : false;
		}
		 ...
		 */
		public function printEntityTableControlResult(array $result, ?int $total = null) {
			$this->setDataType('list');
			$this->setActionType('view');
		...
		/**
		 * Возвращает результат с ошибкой для табличного контрола
		 * @param Exception $exception исключение
		 * @return array
		 */
		public function getEntityTableControlErrorResult(Exception $exception) {
			return [
		...
		/**
		 * Подготавливает значения поля "Набор изображений" или "Набор файлов"
		 * @param mixed $fieldValue значение поля
		 * @param string $typeField тип поля
		 * @return array
		 */
		public function prepareMultipleField($fieldValue, $typeField) {
			$imageList = [];

			foreach ((array) $fieldValue as $key => $value) {
				$imageList[$key] = isset($value['src'])
					? $this->createImageOrFile($typeField, $value['src'], $value)
					: $this->createImageOrFile($typeField, $value);
			}
		 ...
		 * @param iEntityFacade $facade фасад сущностей
		 */
		public function deleteEntityList(array $idList, iEntityFacade $facade) {
			try {
				if (!is_array($idList) || $idList === []) {
					throw new \InvalidArgumentException('Entity id list expected');
				}

				$facade->deleteList($idList);
		...
		/**
		 * Устанавливает опцию сериализации данных
		 * @param string $name имя
		 * @param mixed $value значение
		 */
		public function setTranslatorOption($name, $value) {
			$this->optionList[$name] = $value;
			xmlTranslator::setOption($name, $value);
			jsonTranslator::setOption($name, $value);
		}

		/**
		 * Создает изображение iUmiImageFile или файл iUmiFile
		 * @param string $typeField тип поля
		 * @param string $path путь к изобажению или файлу
		 * @param array $attributeList список атрибутов изображения или файла
		 * @return mixed
		 */
		private function createImageOrFile($typeField, $path, array $attributeList = []) {
			if ($typeField == "multiple_image") {
		...
		/**
		 * Возвращает тип данных по идентификатору
		 * @param int $typeId идентификатор
		 * @return iUmiObjectType
		 * @throws coreException
		 */
		private function getObjectTypeById($typeId) {
		...
		/**
		 * Валидирует поле по имени
		 * @param iUmiObjectType $type тип поля
		 * @param string $name имя поля
		 * @throws coreException
		 * @throws databaseException
		 * @throws publicAdminException
		 */
		private function validateFieldName(iUmiObjectType $type, $name) {
			if ($type->getFieldId($name)) {
				throw new publicAdminException(getLabel('error-non-unique-field-name'));
			}

			$typesCollection = umiObjectTypesCollection::getInstance();
			$parentType = $typesCollection->getType($type->getParentId());

			if ($parentType instanceof iUmiObjectType && $parentType->getFieldId($name)) {
				throw new publicAdminException(
					getLabel('error-non-unique-field-name-in-parent-type', null, $parentType->getName())
				);
			}

			$childList = $typesCollection->getChildTypeIds($type->getId());

			foreach ($childList as $childTypeId) {
				$childType = $typesCollection->getType($childTypeId);

				if ($childType instanceof iUmiObjectType && $childType->getFieldId($name)) {
					throw new publicAdminException(
						getLabel('error-non-unique-field-name-in-child-type', null, $childType->getName())
					);
				}
			}
		}
		...
		public function getCurrentPage() {
			return Service::Request()->pageNumber();
		}
