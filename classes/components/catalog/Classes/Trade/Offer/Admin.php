	...
	class Admin implements \iModulePart {
		...
		public function getProductOfferList($pageId = null, $fieldName = null) {
			...
			try {
				...
				$result = $admin->getEntityTableControlErrorResult($exception);
			}

			$admin->setTranslatorOption('serialize-related-entities', false);
		...
		public function saveTradeOfferField($offerId = null, $field = null, $value = null) {
			...
			try {
				...
				$result = $admin->getSimpleSuccessMessage();
			}  catch (\Exception $exception) {
				$result = $admin->getSimpleErrorMessage($exception);
			}
		...
		private function getTradeOfferListConfig($pageId, $fieldName) {
			...
			return [
				...
				'column-order-key' => $this->getTradeOfferColumnOrderKey($pageId)
			];
		}
		...
		/**
		 * Возвращает ключ для кеша порядка столбцов в таблице торговых предложений
		 * @param int $pageId идентификатор страницы товара
		 * @return string
		 * @throws \expectElementException
		 */
		private function getTradeOfferColumnOrderKey(int $pageId) : string {
			...
			return sprintf('tree-catalog-trade-offer-column-order-for-type-%d', $page->getObjectTypeId());
		}
		...
		private function setCharacteristicValue(iOffer $offer, $field, $value) {
			...
			switch ($characteristic->getFieldType()) {
				case 'date' : {
					$value = $value ? Service::DateFactory()
						->createByDateString($value) : null;
					break;
				}
				case 'file' : {
					$value = $value ? Service::FileFactory()
						->createSecure($value) : null;
					break;
				}
				case 'img_file' : {
					$value = $value ? Service::ImageFactory()
						->create($value) : null;
					break;
				}
				case 'multiple_image' :
				case 'multiple_file': {
					$value = $this->admin->prepareMultipleField($value, $characteristic->getFieldType());
					break;
				}
				case 'symlink' : {
		...
		private function buildPriceFilter(array $offerPriceFilter) {
			...
			foreach ($offerPriceFilter as $typeName => $condition) {
				$priceFilter[] = [
					'type_id' => [
						Filter::COMPARE_TYPE_EQUALS => $this->getPriceType($typeName)->getId()
					],
					'value' => $condition
				];
			}
		...
		private function appendOfferFilterByValueList(array $offerFilter, $name, array $valueList) {
			...
			if (isset($offerFilter[$name][Filter::COMPARE_TYPE_IN_LIST])) {
				$existIdList = $offerFilter[$name][Filter::COMPARE_TYPE_IN_LIST];
				$valueList = array_intersect($existIdList, $valueList);
			}

			$offerFilter[$name] = [
				Filter::COMPARE_TYPE_IN_LIST => $valueList
			];
		...
		private function buildStockBalanceFilter(array $offerStockBalanceFilter) {
			...
			foreach ($offerStockBalanceFilter as $stockIdName => $condition) {
				$priceFilter[] = [
					'stock_id' => [
						Filter::COMPARE_TYPE_EQUALS => $this->getStock($stockIdName)->getId()
					],
					'value' => $condition
				];
			}
		...
		private function buildCharacteristicFilter(array $offerCharacteristicFilter, $typeId) {
			...
			foreach ($offerCharacteristicFilter as $fieldName => $condition) {
				...
				$filter = [
					'name' => [
						Filter::COMPARE_TYPE_EQUALS => $fieldName
					]
				];

				$valueFilter = $condition;

				$fieldTypeName = $characteristic->getFieldType();
				if (contains($fieldTypeName, 'file') || $fieldTypeName === 'boolean' || contains($fieldTypeName, 'multiple')) {
					$isNotNull = (bool) array_shift($condition);
					$compareType = $isNotNull ? Filter::COMPARE_TYPE_NOT_EQUALS : Filter::COMPARE_TYPE_EQUALS;
					$valueFilter = [$compareType => null];
				}

				if ($fieldTypeName === 'relation') {
					$valueFilter = [Filter::COMPARE_TYPE_LIKE => array_shift($condition)];
				}
		...
		private function getOrderIndexSort() {
			return ['order' => Sort::SORT_TYPE_ASC];
		}
