<?php
	...
	class catalog extends def_module {
		...
		public function processPage(iUmiHierarchyElement $page, iUmiHierarchyElement $category, $operation) {
			...
			if ($operation == 'update' && !$this->canBeAddedToIndex($page)) {
				$operation = 'delete';
			}
...
SQL;
		...
		/**
		 * Определяет, может ли товар товар индексироваться для фильтрации
		 * @param iUmiHierarchyElement $page страница товара
		 * @return bool
		 */
		private function canBeAddedToIndex(iUmiHierarchyElement $page) : bool {
			$hideMissingProducts = $this->isNeedHideMissingProducts();
			$productQuantity = (int) $page->getValue('common_quantity');

			if ($hideMissingProducts && $productQuantity <= 0) {
				return false;
			}
		...
		/**
		 * Определяет, нужно ли отключить индексацию товаров, которых нет на складе
		 * @return bool
		 */
		private function isNeedHideMissingProducts() : bool {
			return (bool) Service::Registry()->get('//modules/catalog/disable_missing_products');
		}
	}
