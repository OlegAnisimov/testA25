	...
	class core {
		...
		public function navibar($template = 'default', $isFull = true, $offsetLeft = 0, $offsetRight = 0) {
			...
			for ($i = (int) $offsetLeft; $i < $sz; $i++) {
				...
				$item_arr = def_module::parseTemplate(
					$tpl_item_current,
					[
		...
		public function getTypeEditLinkXml($typeId) {
			...
			if (permissionsCollection::getInstance()->isAllowedMethod($userId, 'data', 'type_edit')) {
		...
		/**
		 * Подключает кастомные файлы выгрузки в сценарий выгрузки
		 * Добавлен для обратной совместимости
		 * @param string $type import или export
		 * @param string $fileName имя файла сценария выгрузки
		 * @return array
		 */
		public function includeExchangeXsl($type, $fileName) {
			$path = CURRENT_WORKING_DIR . "/xsl/$type/custom/$fileName.xsl";

			if (!is_file($path)) {
				$path = CURRENT_WORKING_DIR . "/styles/common/xsl/stub.xsl";
			}
		...
		public function contextManualUrl() {
			$cmsController = cmsController::getInstance();
			/** @var def_module|baseModuleAdmin $module */
			$module = $cmsController->getModule($cmsController->getCurrentModule());
			return $module->getManualUrl();
		}
		...
		/** @return array */
		public function getLangsList() : array {
			return ['langs' => ['nodes:lang' => Service::LanguageCollection()->getList()]];
		}
		...
		public function insertPopup($text = '', $src = '') {
			...
			if (file_exists($path)) {
				...
				if (is_array($isz)) {
					...
					$res =
						"<a href=\"$src\" onclick=\"javascript: return gen_popup('$src', '$width', '$height');\" class=\"umi_popup\">" .
						$text . '</a>';
				}
			}
			return $res;
		...
		public function insertThumb($src1 = '', $src2 = '', $alt = '') {
			...
			if (file_exists($path2)) {
				...
				if (is_array($isz)) {
					list($width, $height) = $isz;
					$res = "<a href=\"$src2\" onclick=\"javascript: return gen_popup('$src2', '$width', '$height');\">" .
						$thumb . '</a>';
				}
			}
			return $res;
