<?php
	...
	class EmarketAdmin {
		 ...
		 * @param int|null $paymentId идентификатор способа оплаты
		 * @param int|null $orderId идентификатор заказа
		 * @throws ErrorException
		 * @throws coreException
		 * @throws publicException
		 */
		public function approvePayment(int $paymentId = null, int $orderId = null) {
			$payment = $this->getPayment($paymentId, $orderId);
			$this->setData(['message' => $payment->approvePayment()]);
			$this->doData();
		}

		/**
		 * Отменяет оплату заказа
		 * @param int|null $paymentId идентификатор способа оплаты
		 * @param int|null $orderId идентификатор заказа
		 * @throws ErrorException
		 * @throws coreException
		 * @throws publicException
		 */
		public function cancelPayment(int $paymentId = null, int $orderId = null) {
			$payment = $this->getPayment($paymentId, $orderId);
			$this->setData(['message' => $payment->cancelPayment()]);
			$this->doData();
		}

		/**
		 * Возвращает средства клиенту по оплате
		 * @param int|null $paymentId идентификатор способа оплаты
		 * @param int|null $orderId идентификатор заказа
		 * @throws ErrorException
		 * @throws coreException
		 * @throws publicException
		 */
		public function refundPayment(int $paymentId = null, int $orderId = null) {
			$payment = $this->getPayment($paymentId, $orderId);
			$this->setData(['message' => $payment->refundPayment()]);
			$this->doData();
		}
		 ...
		 * @throws publicException
		 */
		private function getPayment(int $paymentId = null, int $orderId = null) : payment {
			$post = Service::Request()->Post();
			$orderId = $orderId ?: $post->get('order_id');

			if (!Service::ObjectsCollection()->isExists($orderId)) { // prevent order creating if incorrect id
				throw new ErrorException('Incorrect order id given');
			}

			$order = order::get($orderId, true);

			if (!$order instanceof order) {
				throw new ErrorException(sprintf('Cannot get order by id "%s"', $orderId));
			}

			$paymentId = $paymentId ?: $post->get('payment_id');
			$payment = payment::get($paymentId, $order);

			if (!$payment instanceof payment) {
				throw new ErrorException(sprintf('Cannot get payment by id "%s"', $paymentId));
			}
		...
		public function config() {
			...
			$params = [
				'emarket-options' => [
					...
					'boolean:put-any-type-pages-into-basket' => null,
					'boolean:is-discount-without-rules-works-for-all-goods' => null,
			...
			if ($this->isSaveMode()) {
				...
				$umiRegistry->set(
					'//modules/emarket/drop-customer-after-order-execute',
					$params['emarket-options']['boolean:drop-customer-after-order-execute']
				);
				$umiRegistry->set(
				...
				$umiRegistry->set(
					'//modules/emarket/put-any-type-pages-into-basket',
					$params['emarket-options']['boolean:put-any-type-pages-into-basket']
				);
				$umiRegistry->set(
					'//modules/emarket/is-discount-without-rules-works-for-all-goods',
					$params['emarket-options']['boolean:is-discount-without-rules-works-for-all-goods']
				);
			...
			$params['emarket-options']['boolean:drop-customer-after-order-execute'] =
				$umiRegistry->get('//modules/emarket/drop-customer-after-order-execute');
			$params['emarket-options']['boolean:merge-guest-orders-for-admins'] =
			...
			$params['emarket-options']['boolean:put-any-type-pages-into-basket'] =
				$umiRegistry->get('//modules/emarket/put-any-type-pages-into-basket');
			$params['emarket-options']['boolean:is-discount-without-rules-works-for-all-goods'] =
				$umiRegistry->get('//modules/emarket/is-discount-without-rules-works-for-all-goods');
		...
		public function flushDefaultStoreAttributes() {
			...
			foreach ($settingKeyList as $settingKey) {
				$result[$settingKey] = $settings->get(Settings::DEFAULT_STORE_SECTION, $settingKey);
			}
		 ...
		 * @param int|null $customerId идентификатор покупателя
		 * @return array
		 * @throws selectorException
		 */
		public function getCustomerOrderList($customerId = null) {
			if (!Service::Request()->isJson()) {
				$this->setDataSetDirectCallMessage();
				return;
			}
			$customerId  = $customerId  ?: $this->getNumberedParameter(0);

			$sel = Service::SelectorFactory()->createObjectTypeName('emarket', 'order');
			...
			$sel->limit($offset, $limit);
			$tolalCount = $sel->total;

			$result = [];
			$typeId = umiObjectTypesCollection::getInstance()->getTypeIdByGUID("emarket-orderstatus");
			$statuses = umiObjectsCollection::getInstance()->getGuidedItems($typeId);
			foreach ($sel->result() as $order) {
				$item['id'] = $order->getId();
				$item['number'] = $order->getName();
				$item['order_date'] = $order->getValue('order_date')->getFormattedDate();
				$statusId = $order->getValue('status_id');
				$item['status'] = $statuses[$statusId];
				$result[] = $item;
			}

			$result = $this->appendPageNavigation($result, $tolalCount);
			$this->printEntityTableControlResult($result);
		}

		/** 
		 * Возвращает конфиг для табличного контрола списка заказов покупателя 
		 * @param int|null $customerId идентификатор покупателя
		 */
		public function flushOrderListConfig($customerId = null) {
			$customerId  = $customerId  ?: $this->getNumberedParameter(0);
			Service::Response()->printJson([
				'methods' => [
					[
						'title' => getLabel('smc-load'),
						'module' => 'emarket',
						'type' => 'load',
						'name' => sprintf('getCustomerOrderList/%d', $customerId)
					],
					[
						'title' => getLabel('header-users-edit-order'),
						'module' => 'emarket',
						'type' => 'edit',
						'name' => 'order_edit'
					],
				],
				'default' => 'number[250px]|order_date[300px]|status[300px]',
				'fields' => [
					[
						'name' => 'number',
						'title' => getLabel('label-orders-name'),
						'type' => 'string',
					...
					[
						'name' => 'order_date',
						'title' => getLabel('label-orders-date'),
						'type' => 'string',
					...
					[
						'name' => 'status',
						'title' => getLabel('label-orders-status'),
						'type' => 'string',
						'show_edit_page_link' => 'false',
						'filterable' => 'false',
						'sortable' => 'false',
