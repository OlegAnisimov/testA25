<?php
	 ...
	 * 10) Интеграция сервиса Яндекса "Быстрые заказ";
	 * 11) Сбор базовой статистики и предоставление отчетов;
	...
	class emarket extends def_module {
		...
		const SETTINGS_CLASS = 'UmiCms\Classes\Components\Emarket\Config\Settings';
		...
		public function __construct() {
			...
			$this->__loadLib('classes/Config/Settings.php');
			$this->__implement('UmiCms\Classes\Components\Emarket\Config\Settings');
			$this->__implement('EmarketSettings'); // обратная совместимость
		...
		public function includeAdminClasses() {
			...
			$this->__loadLib('classes/Config/DeliveryTab.php');
			$this->__implement('UmiCms\Classes\Components\Emarket\Config\DeliveryTab');
			$this->__implement('EmarketDeliverySettingsAdmin'); // обратная совместимость

			$this->__loadLib('classes/Config/PaymentTab.php');
			$this->__implement('UmiCms\Classes\Components\Emarket\Config\PaymentTab');
			$this->__implement('EmarketPaymentSettingsAdmin'); // обратная совместимость

			$this->__loadLib('classes/Config/PurchasingTab.php');
			$this->__implement('UmiCms\Classes\Components\Emarket\Config\PurchasingTab');
		...
		/**
		 * Возвращает валидатор заказов
		 * @return iValidator
		 * @throws Exception
		 */
		public function getOrderValidator() {
			return Service::get('OrderValidator');
		}
		...
		public function getProductWeightFieldId() {
			return (int) $this->getSettings()->get(Settings::ORDER_ITEM_SECTION, 'weightField');
		}
		...
		/**
		 * Определяет, доступно ли использование скидок с правилом "По промокоду"
		 * @return bool
		 * @throws selectorException
		 */
		public function isPromoCodesAvailable() : bool {
			...
			$discountObjects = $this->getAllDiscounts();

			/** @var iUmiObject $object */
			foreach ($discountObjects as $object) {
				$discount = discount::get($object->getId());
				...
				foreach ($rules as $rule) {
					if ($rule instanceof promoCodeDiscountRule) {
						return true;
					}
				}
			}
		...
		/**
		 * Возвращает идентификатор ставки НДС по-умолчанию
		 * @return int|null
		 */
		public function getDefaultVatId() {
			try {
				return (int) $this->getSettings()->get(Settings::ORDER_ITEM_SECTION, 'taxRateId');
		...
		/** 
		 * Проверяет включены ли скидки в системе 
		 * @return bool
		 */
		private function isEnabledDiscounts() {
			return Service::Registry()->get('//modules/emarket/enable-discounts');
		}
