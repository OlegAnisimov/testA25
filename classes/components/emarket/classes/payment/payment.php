<?php
	 ...
	 *
	 * 1) "Наличными курьеру";
	 * 2) "Деньги Online" (С поддержкой ФЗ-54);
	 * 3) "Счет для юридических лиц";
	 * 4) "PayAnyWay" (С поддержкой ФЗ-54);
	 * 5) "PayOnline System (С поддержкой ФЗ-54)";
	 * 6) "PayPal";
	 * 7) "Платежная квитанция";
	 * 8) "Robokassa" (С поддержкой ФЗ-54);
	 * 9) "Яндекс.Касса" (С поддержкой ФЗ-54);
	 *
	...
	abstract class payment extends umiObjectProxy {
		...
		public function approvePayment() : string {
			throw new ErrorException(getLabel('error-label-payment-method-not-implemented'));
		}
		...
		/**
		 * Алиас @see payment::isPaymentDisabledInOrder()
		 * Проверяет, подходит ли конкретная платежная система для использования.
		 * Результат зависит от от особенностей работы этой платежной системы.
		 * @param order $order заказ
		 * @return bool
		 */
