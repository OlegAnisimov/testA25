<?php
	...
	class EmarketHandlers {
		...
		public function onCronLoadCurrencyInfo($filePath) {
			...
			$xslPath = CURRENT_WORKING_DIR
				. '/styles/common/xsl/currencies/'
				. $config->get('modules', 'emarket.currency.sync.xsl');
		...
		public function onCronCheckExpiredCustomers(iUmiEventPoint $event) {
			...
			foreach ($customers as $customerId) {
				...
				$customerDataObject = $objects->getObject($customerId);
		...
		/**
		 * Исправляет значения в полях способа оплаты
		 * Удаляет пробелы из начала и конца строковых полей из группы "Параметры"
		 * @param iUmiEventPoint $event событие сохранения или изменения объекта
		 * @return bool
		 * @throws databaseException
		 */
		public function fixPaymentSettings(iUmiEventPoint $event) : bool {
			if ($event->getMode() != 'after') {
				return false;
			}
			...
			$paymentRootTypeId = $typesCollection->getTypeIdByHierarchyTypeName('emarket', 'payment');

			if ($typesCollection->getParentTypeId($typeId) != $paymentRootTypeId) {
				return false;
			}

			$fieldIdList = $object->getPropGroupByName('settings');
			...
			foreach ($fieldIdList as $id) {
				$field = $fieldsCollection->getById($id);
				...
				$object->setValue($fieldName, trim($value));
			}
