<?php
	 ...
	 *
	 * 15) Ожидание завершения оформления;
	 * @link /emarket/purchase/result/wait
	 *
	 * С этапами оформления можно ознакомиться в классе EmarketPurchasingStages.
	 */
	class EmarketPurchasingStagesSteps {

		/** @var emarket|EmarketPurchasingStages $module */
		public $module;
		...
		public function savePersonalInfo($order) {
			...
			$this->module->redirectToPurchaseUrl(['delivery', 'address']);
		}
		...
		public function chooseDeliveryAddress(order $order) {
			$selfDeliveryRequired = (bool) Service::Registry()
				->get('//modules/emarket/delivery-with-address');

			$deliveryId = getRequest('delivery-id');

			if ($selfDeliveryRequired && $deliveryId) {
				$delivery = delivery::get($deliveryId);

				if ($delivery instanceof selfDelivery) {
					$order->setDelivery($delivery);
					$order->commit();
					$this->module->redirectToPurchaseUrl(['payment', 'bonus']);
				}
			}
			...
			$this->module->redirectToPurchaseUrl(['delivery', 'choose']);
		}
		...
		public function choosePayment(order $order) {
			...
			$this->module->redirectToPurchaseUrl(['payment', $paymentName]);
		}
		...
		public function payByBonus(order $order) {
			...
			if ($bonus || $bonus === 0 || $bonus === '0') {
				...
				if (!$order->getActualPrice()) {
					...
					$order->order();
					$this->module->redirectToSuccessfulResult();
				}
			}

			$this->module->redirectToPurchaseUrl(['payment', 'choose']);
		}
		...
		public function formResult(order $order, $stage, $mode, $template) {
			list($tplSuccessful, $tplFailed, $tplWait) = emarket::loadTemplates(
				'emarket/' . $template,
				'purchase_successful',
				'purchase_failed',
				'purchase_wait'
			...
			$tplBlock = $tplWait;
			$paymentCode = order::getCodeByStatus($order->getPaymentStatus());

			if ($stage === 'wait' && in_array($paymentCode, ['accepted', 'declined'])) {
				$this->redirectByPaymentCode($paymentCode);
			}

			switch ($stage) {
				case 'successful': {
					$tplBlock = $tplSuccessful;
					break;
				}
				case 'fail': {
					$tplBlock = $tplFailed;
			...
			$invoiceLink = ($payment instanceof invoicePayment) ? $payment->getInvoiceLink() : '';
			...
			$isDropCustomerAfterOrderExecute = Service::Registry()
				->get('//modules/emarket/drop-customer-after-order-execute');

			if ($isGuest && ($stage === 'successful') && $isDropCustomerAfterOrderExecute) {
		...
		/**
		 * Производит перенаправление на страницу результата оплаты
		 * @param string $paymentCode статус оплаты
		 * @throws coreException
		 */
		private function redirectByPaymentCode($paymentCode) {
			switch ($paymentCode) {
				case 'accepted': {
					$this->module->redirectToSuccessfulResult();
					break;
				...
				case 'declined': {
					$this->module->redirectToFailResult();
					break;
				}
			}
		}
