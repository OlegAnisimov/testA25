<?php
	...
	namespace UmiCms\Classes\Components\Filemanager;
	...
	class Filemanager {
		...
		/** @var Filemanager|\def_module|\filemanager $module */
		public $module;
		...
		public function elfinder_connector($needInfo = false) {
			...
			$request = Service::Request()->Get();

			/** @var \iUmiEventPoint $eventPoint */
			$eventPoint = Service::EventPointFactory()
				->create('filemanager_options_create', 'after');
			$eventPoint->setParam('options', $opts);
			$eventPoint->setParam('page_id', $request->get('page_id'));
			$eventPoint->setParam('object_id', $request->get('object_id'));
			$eventPoint->setParam('field_name', $request->get('field_name'));
			$eventPoint->call();
