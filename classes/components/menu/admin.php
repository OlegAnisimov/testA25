<?php
	...
	class MenuAdmin {
		...
		public function lists(int $domainId  = null, int $languageId = null) {
			...
			$sel->option('or-mode')->field('domain_id_list', 'language_id_list');
			$sel->where('domain_id_list')->equals($domainId);
			$sel->where('domain_id_list')->isnull();
			$sel->where('language_id_list')->equals($languageId);
			$sel->where('language_id_list')->isnull();
