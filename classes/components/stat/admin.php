...
END;
		...
		public function config() {
			...
			$params = [
				'config' => [
					'boolean:stat-enabled' => null
				]
			];
			$params['config']['boolean:stat-enabled'] = (boolean) $regedit->get('//modules/stat/collect');

			if ($this->isSaveMode()) {
				$params = $this->expectParams($params);
				$regedit->set('//modules/stat/collect', $params['config']['boolean:stat-enabled']);
