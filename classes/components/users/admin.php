...
/** Класс функционала административной панели */
	class UsersAdmin {
		...
		const TYPE_NAME_LIST = ['user', 'users', 'author'];
		...
		/**
		 * Возвращает список авторов с учетом фильтров
		 * @throws coreException
		 * @throws expectObjectException
		 * @throws selectorException
		 */
		public function authorsList() {
			...
			$selector = Service::SelectorFactory()->createObjectTypeName('users', 'author');
			...
			$this->setDataRange($limit, $offset);
			$data = $this->prepareDataObjects($selector->result());
		...
		public function add() {
			...
			$inputData = [
				...
				'aliases' => ['name' => 'login'],
				'allowed-element-types' => self::TYPE_NAME_LIST
			];
			...
			if ($this->isSaveMode('param1')) {
				...
				$this->savePermissions($object->getId(), $object->getTypeGuid(), __FUNCTION__);
		...
		public function edit() {
			...
			if ($this->isSaveMode('param1')) {
				...
				$this->savePermissions($objectId, $object->getTypeGuid(), __FUNCTION__);
				$this->chooseRedirect();
			}
		 ...
		 * @param int|null $id id пользователя|группы
		 * @param int|null $element_id id страницы
		 */
		public function getUserPermissions($id = null, $element_id = null) {
			$id = $id ?: $this->getNumberedParameter(0);
			$element_id = $id ?: $this->getNumberedParameter(1);
		...
		public function choose_perms($ownerId = false) {
			...
			foreach ($modules_list as $md) {
				...
				if (!$permissions->isAllowedModule($userId, $module_name)) {
					continue;
				}
				...
				if (is_array($func_list)) {
					foreach ($func_list as $method_name) {
						if (!$permissions->isAllowedMethod($userId, $module_name, $method_name)) {
							continue;
						}
		...
		/**
		 * Сохраняет права пользователя|группы
		 * @param int $objectId id объекта пользователя|группы
		 * @param string $objectTypeGuid Guid типа объекта
		 * @param string $functionName имя функции которая запрашивает изменения прав
		 */
		private function savePermissions($objectId, $objectTypeGuid, $functionName) {
			if ($objectTypeGuid != 'users-author') {
				$this->module->save_perms($objectId, $functionName);
			}
		}
	}
