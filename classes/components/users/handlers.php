<?php
	...
	class UsersHandlers {
		...
		/**
		 * Выполняет редирект после успешной авторизации
		 * @param iUmiEventPoint $event событие успешной авторизации
		 * @throws privateException
		 */
		public function onLoginSuccess(iUmiEventPoint $event) {
			...
			system_get_skinName();
			$redirectToDefaultModule = Service::Registry()->get('//settings/default_module_admin_changed');

			if ($redirectToDefaultModule) {
				$module = $this->module->getModuleForAdminEntrance();
				$address = sprintf('%s/admin/%s/', $this->module->pre_lang, $module);
			} else {
				$url = getRequest('from_page');
				$address = $url ?: Service::Request()->referrer();
			}

			$address = $address ?: $this->module->pre_lang . '/admin/';
			$this->module->redirect($address);
		}
	}
