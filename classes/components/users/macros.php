	...
	class UsersMacros {
		...
		public function logout($makeRedirect = true) {
			...
			if ($makeRedirect) {
				...
				$this->module->redirect($redirect_url);
			}
		}
		...
		public function getModuleForAdminEntrance() {
			$moduleName = Service::Registry()->get('//settings/default_module_admin') ?: 'events';
		...
		public function getLoginRedirectUrl() {
			...
			$url = $url ?: Service::Request()->referrer();
		...
		public function loginza() {
			...
			$provider_name = $profile->genProviderName();
			...
			$this->tryToLoginAsExistingSocialUser($profile, 'loginza');
		...
		public function ulogin() {
			...
			$uid = $data->uid;
			$this->findAndUpdateUidUser($data);
			...
			try {
				$this->tryToLoginAsExistingSocialUser($data, 'ulogin');
		...
		/**
		 * Пытается авторизовать пользователя, который логинится через социальную сеть.
		 * @param Object $data данные пользователя
		 * @param string $service loginza или ulogin
		 * @return void
		 * @throws ErrorException
		 * @throws coreException
		 * @throws errorPanicException
		 ...
		 * @throws selectorException
		 */
		public function tryToLoginAsExistingSocialUser($data, $service = null) : void {
			$module = $this->module;
			...
			if ($service == 'ulogin') {
				...
				$uid = $data->uid;
				$email = $data->email;
				$isUniqueEmail = $module->checkIsUniqueEmail($email);
				$isEmailVerified = $this->isEmailVerified($data, 'ulogin');
			}

			if ($service == 'loginza') {
				$network = $data->genProviderName();
				$uid = $data->genNickname() . '@' . $network;
				$email = $data->genUserEmail();
				$isUniqueEmail = $module->checkIsUniqueEmail($email);
				$isEmailVerified = $this->isEmailVerified($data, 'loginza');
			}

			if (!$isEmailVerified) {
				$module->errorNewMessage(getLabel('error-not-verified-email'));
				$module->errorPanic();
			}

			if (!$isUniqueEmail) {
				$userId = $this->getUserIdByEmail($email);
				$result = Service::Auth()->loginUsingId($userId);
			} else {
				$result = Service::Auth()->loginBySocials($uid, $network, $service);
			}
		...
		public function createNewSocialUserAndLogin(array $data) {
			...
			$user->setValue('social_uid', $data['uid']);
		...
		public function list_users($template = 'default', $perPage = 10) {
			...
			$blockArr = [];
			$currPage = Service::Request()->pageNumber();
		...
		/**
		 * Поиск пользователя по логину и названию социальной сети 
		 * и обновление его идентификатора в социальной сети
		 * @param Object $data
		 * @throws selectorException
		 */
		private function findAndUpdateUidUser($data) {
			$network = $data->network;
			$login = $data->nickname . '@' . $network;
			$uid = $data->uid;

			$sel = Service::SelectorFactory()
			...
			if ($data->verified_email == 1) {
				$sel->where('e-mail')->equals($data->email);
			} else {
				$sel->where('fname')->equals($data->first_name);
			}
			$sel->where('is_activated')->equals(true);
			$objectUser = $sel->first();

			if (!$objectUser instanceof iUmiObject) {
				return;
			}

			$objectUser->setValue('social_uid', $uid);
			$objectUser->commit();
		}
		...
		private function getUserIdByEmail(string $email) : int {
			...
			if (!$sel->first() instanceof iUmiObject) {
				throw new publicException(getLabel('label-no-users-found'));
			}
		...
		/**
		 * Проверяет, верифицирован ли email в социальной сети
		 * @param Object $data данные пользователя
		 * @param string $service loginza или ulogin
		 * @return bool
		 */
		private function isEmailVerified(Object $data, string $service) : bool {
			if ($service == 'ulogin') {
				return $data->verified_email == 1;
			}

			switch ($data->genProviderName()) {
				case 'plus.google.com':
				case 'login.yandex.ru':
				case 'mail.ru':
					return true;
				default:
					return false;
