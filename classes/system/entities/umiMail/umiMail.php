<?php
	...
	class umiMail implements iUmiMail {
		...
		/** @var string $listUnsubscribe заголовок List-Unsubscribe для отписки от рассылки */
		...
		/** @var bool $parseContent флаг необходимости генерации текстовой версии письма */
		private $generateTxtBody = true;
		...
		/** @const string GENERATE_TXT_BODY опция генерации текстовой версии письма */
		const GENERATE_TXT_BODY = 'generate-txt-body';

		/** @const string X_MAILER заколовок X-mailer */
		const X_MAILER = 'UMI.CMS';
		...
		public function __construct($template = 'default', $engine = null) {
			...
			$generateTxtBody = $this->getConfigMailValue(self::GENERATE_TXT_BODY);

			if ($generateTxtBody !== null) {
				$this->generateTxtBody = (bool) $generateTxtBody;
			}
		...
		public function setListUnsubscribe($listUnsubscribe) {
			if (is_string($listUnsubscribe)) {
				$listUnsubscribe = [$listUnsubscribe];
			}

			$this->listUnsubscribe = '<' . implode(">, <", $listUnsubscribe) . '>';
			return $this;
		}

		/** @inheritDoc */
		public function setGenerateTxtBody($flag) {
			$this->generateTxtBody = $flag;
			return $this;
		}
		...
		private function attachInnerImages($content) {
			...
			$firstPattern = '#<\w+[^>]+\s((?i)src|background|href(?-i))\s*=\s*(["\']?)?([\w\?=\.\-_:\/]+.(jpeg|jpg|gif|png|bmp|webp))\2#i';
			...
			$secondPattern = '#(?i)url(?-i)\(\s*(["\']?)([\w\.\-_:\/]+.(jpeg|jpg|gif|png|bmp|webp))\1\s*\)#';
		...
		public function send() {
			...
			$this->arrHeaders['X-Mailer'] = $this->getXMailer();
			...
			if ($this->listUnsubscribe) {
				$this->arrHeaders['List-Unsubscribe'] = $this->listUnsubscribe;
			}
			...
			if ($this->generateTxtBody && $bNeedHtmlBody) {
				$this->sTxtBody = strip_tags($this->content);
				$bNeedTxtBody = true;

				$event = Service::EventPointFactory()
					->create('systemGenerateMailTxtBody', 'after')
					->setParam('htmlBody', $this->content)
					->setParam('parseHtmlBody', $content)
					->setParam('txtBody', $this->sTxtBody)
					->setParam('needTxtBody', $bNeedTxtBody);

				$event->call();

				$content = $event->getParam('parseHtmlBody');
				$this->sTxtBody = $event->getParam('txtBody');
				$bNeedTxtBody = $event->getParam('needTxtBody');
			}
		...
		/**
		 * Возвращает значение заголовка X-Mailer
		 * @return string
		 * @throws coreException
		 */
		private function getXMailer() {
			if ($this->isUseCustomSettings()) {
				$registryPath = $this->getCustomPrefix() . 'x-mailer';
				$xMailer = Service::Registry()->get($registryPath);

				return !empty($xMailer) ? $xMailer : self::X_MAILER;
			}

			$xMailer = $this->getConfigMailValue('x-mailer');
			return !empty($xMailer) ? $xMailer : self::X_MAILER;
		}
