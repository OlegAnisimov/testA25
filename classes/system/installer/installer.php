<?php
	...
	if (mb_strpos(__DIR__, 'classes/system/installer') !== false) {
		define('INSTALLER_CURRENT_WORKING_DIR', realpath(__DIR__ . '/../../..'));
	...
	class umiInstallExecutor {
		...
		/** @var string NEWEST_VERSION_ID идентификатор последней версии */
		const NEWEST_VERSION_ID = 'last';

		/** Файл, в котором хранится состояние обновления */
		const STATE_FILE_NAME = '.isf';

		/** @var string HTACCESS_START_MARK метка начала системных правил в .htaccess */
		const HTACCESS_START_MARK = '####################### UMI_CMS_HTACCESS_BEGIN ###########################';

		/** @var string HTACCESS_END_MARK метка конца системных правил в .htaccess */
		const HTACCESS_END_MARK = '######################## UMI_CMS_HTACCESS_END ############################';
		...
		public function run($currentStep = 'install-run', $cli = true, $params = []) {
			...
			try {
				switch ($currentStep) {
					...
					// Предоставляет возможность выбрать версию
					case 'select-version':
						$result = $this->getAvailableVersions();
						break;
		...
		private static function getBacktraceXML(DOMDocument $document, $trace) {
			...
			foreach ($trace as $callInfo) {
				...
				$callString = isset($callInfo['class']) ? $callInfo['class'] : '' .
					isset($callInfo['type']) ? $callInfo['type'] : '' .
					isset($callInfo['function']) ? $callInfo['function'] : '' .
					"({$arguments})";
		...
		private function getDownloader() {
			if ($this->installMode) { // Режим установки
				...
				$current_revision = self::NEWEST_VERSION_ID;
			} else { // Режим обновления, в нем мы всегда в папке /classes/system/installer/
				$this->includeCore();
				...
				$current_revision = $umiRegistry->get('//settings/system_build') ?:  self::NEWEST_VERSION_ID;
				$host = Service::DomainCollection()->getDefaultDomain()->getHost();
			}

			$targetRevision = $this->getSelectedVersion();
			$downloader = new umiUpdateDownloader($this->temporaryDirectory, $key, $host, $ip, $current_revision, $targetRevision);
			return $downloader;
		}
		...
		/**
		 * Обвновляет корневой .htaccess
		 * @throws Exception
		 */
		private function updateHtaccess() {
			$newHtaccessPath = $this->temporaryDirectory . '/installer.service/.htaccess.original';
			$oldHtaccessPath = './.htaccess';

			if (!file_exists($newHtaccessPath)) {
				throw new Exception('В сервисном пакете отсутствует корневой файл настроек .htaccess');
			}

			$newHtaccess = trim((string) file_get_contents($newHtaccessPath));
			$oldHtaccess = file_exists($oldHtaccessPath) ? trim((string) file_get_contents($oldHtaccessPath)) : '';

			if ($oldHtaccess && ($oldHtaccess !== $newHtaccess) && !$this->installMode) {
				if (!copy($oldHtaccessPath, './.htaccess_old')) {
					throw new Exception('Не удается сохранить предыдущий .htaccess, проверьте разрешения!');
				}
			}

			if (contains($oldHtaccess, self::HTACCESS_START_MARK) && contains($oldHtaccess, self::HTACCESS_END_MARK)) {
				$htaccessStartMarkPosition = stripos($oldHtaccess, self::HTACCESS_START_MARK);
				$customHtaccessHeader = ($htaccessStartMarkPosition > 0) ? substr($oldHtaccess, 0, $htaccessStartMarkPosition) : '';
				$customHtaccessHeader = ltrim($customHtaccessHeader);

				$htaccessEndMartPosition = stripos($oldHtaccess, self::HTACCESS_END_MARK);
				$customHtaccessFooter = substr($oldHtaccess, $htaccessEndMartPosition);
				$customHtaccessFooter = str_replace(self::HTACCESS_END_MARK, '', $customHtaccessFooter);
				$customHtaccessFooter = rtrim($customHtaccessFooter);

				$newHtaccess = trim($customHtaccessHeader . $newHtaccess . $customHtaccessFooter);
			}

			if (!file_put_contents($oldHtaccessPath, $newHtaccess)) {
		...
		private function updateInstaller() {
			...
			$installer = $this->temporaryDirectory . '/core/classes/system/installer/installer.php';
		...
		private function includeCore() {
			if ($this->isCmsUpdating() && !$this->installMode) {
				$this->connection = ConnectionPool::getInstance()->getConnection();
				return;
			}

			$core = INSTALLER_CURRENT_WORKING_DIR . '/standalone.php'; //legacy include core logic
		...
		/**
		 * Определяет было ли запущено обновлени
		 * @return bool
		 */
		private function isCmsUpdating() {
			return file_exists('./updating');
		}
		...
		private function updateDatabaseStructure() {
			...
			$database_structure = $this->temporaryDirectory . 'installer.service/database.xml';
		...
		private function installComponent() {
			...
			$componentDir = "{$this->temporaryDirectory}/{$componentName}";

			if (file_exists("{$componentDir}/config.distr.ini")) {
				mainConfiguration::getInstance()->loadConfig("{$componentDir}/config.distr.ini")->save();
			}

			$componentConfig = "{$componentDir}/{$componentName}.xml";
			...
			if (INSTALLER_DEBUG || !INSTALLER_CLI_MODE) {
				$log = $importer->getImportLog();
				if (count($log) > 0) {
					$logPath = INSTALLER_CURRENT_WORKING_DIR . '/install.log';
					file_put_contents($logPath, implode(PHP_EOL, $log), FILE_APPEND);
				}
			}

			if ($componentName === 'core') {
				$this->copySpecialFiles();
			}
		...
		/** Копирует особые файлы ядра */
		private function copySpecialFiles() {
			$specialDirectoryPathList = [
				"{$this->temporaryDirectory}/core/classes/vendor/",
				"{$this->temporaryDirectory}/core/classes/system/autoload/",
				"{$this->temporaryDirectory}/core/classes/system/subsystems/Deprecated/",
				"{$this->temporaryDirectory}/core/classes/system/subsystems/models/events/",
				"{$this->temporaryDirectory}/core/classes/system/subsystems/services/",
				"{$this->temporaryDirectory}/core/classes/system/subsystems/Cookies/",
				"{$this->temporaryDirectory}/core/classes/system/subsystems/protection/",
				"{$this->temporaryDirectory}/core/classes/system/subsystems/selector/",
				"{$this->temporaryDirectory}/core/classes/system/subsystems/Response/",
				"{$this->temporaryDirectory}/core/classes/system/subsystems/Request/",
				"{$this->temporaryDirectory}/core/classes/system/subsystems/models/hierarchy/",
				"{$this->temporaryDirectory}/core/classes/system/utils/Html/",
				"{$this->temporaryDirectory}/core/classes/system/utils/Api/Http/Json/Google/",
				"{$this->temporaryDirectory}/core/classes/system/utils/pagenum/",
				"{$this->temporaryDirectory}/core/classes/system/utils/Url/",
			];
			$temporaryDir = "{$this->temporaryDirectory}/core";

			foreach ($specialDirectoryPathList as $specialDirectoryPath) {
				$specialDirectory = new umiDirectory($specialDirectoryPath);
				$newSpecialDirectory = str_replace($temporaryDir, INSTALLER_CURRENT_WORKING_DIR, $specialDirectoryPath);
				umiDirectory::requireFolder($newSpecialDirectory, INSTALLER_CURRENT_WORKING_DIR);

				$directoryType = 2;
				foreach ($specialDirectory->getAllFiles($directoryType) as $directoryPath => $directoryName) {
					$newDirectoryPath = str_replace($temporaryDir, INSTALLER_CURRENT_WORKING_DIR, $directoryPath);
					umiDirectory::requireFolder($newDirectoryPath, INSTALLER_CURRENT_WORKING_DIR);
				}

				$fileType = 1;
				foreach ($specialDirectory->getAllFiles($fileType) as $filePath => $fileName) {
					$newFilePath = str_replace($temporaryDir, INSTALLER_CURRENT_WORKING_DIR, $filePath);
		...
		private function setDefaultDomain() {
			...
			if (Service::Request()->isHttps()) {
				$defaultDomain->setUsingSsl();
			}
		...
		private function configure() {
			...
			$selectedBuild = $this->getSelectedVersion();
			$packageVersion = $version ? $version->getAttribute('name') : '';
			$packageRevision = $package->getAttribute('last-revision');
			$packageBuild = sprintf('%s_%s', $packageVersion, $packageRevision);

			if ($selectedBuild !== self::NEWEST_VERSION_ID && $selectedBuild !== $packageBuild) {
				$matches = [];

				if (!preg_match('|(\d{1,})_(\d{1,})|', $selectedBuild, $matches)) {
					throw new Exception(sprintf('Передана некорректная версия: "%s"', $selectedBuild));
				}

				$packageVersion = (string) $matches[1];
				$packageRevision = (string) $matches[2];
			}
			...
			$umiRegistry->set('//settings/system_version', $packageVersion);
			$umiRegistry->set('//settings/system_build', $packageRevision);
			$umiRegistry->set('//settings/last_updated', $package->getAttribute('generated'));
			...
			if ($this->installMode) {
				try {
					$this->sendEmailWithCredentials($email, $firstName, $login, $password);
				} catch (Exception $e) {
					//nothing
				}
			}
		...
		/** 
		 * Отправляет письмо с доступами на почту супервайзера 
		 * @param string $email почта супервайзера
		 * @param string $name имя супервайзера
		 * @param string $login логин супервайзера
		 * @param string $password пароль супервайзера
		 * */
		private function sendEmailWithCredentials($email, $name, $login, $password) {
			$domain = Service::DomainCollection()->getDefaultDomain();
			$host = $domain->getHost();
			$siteUrl = $domain->getUrl();
			
			$content = <<<MSG
<p>
	Здравствуйте, $name<br/>
	Вы установили систему UMI.CMS на домене <a href='$siteUrl'>$host</a>.
</p>
<p>
	Доступы в административную панель<br/>
	<a href='$siteUrl/admin'>$host/admin</a><br/>
	Логин: $login<br/>
	Пароль: $password
</p>
MSG;
			...
			$mail->setSubject("UMI.CMS успешно установлена");
			$mail->setContent($content);
			$mail->setNeedToParseContent(false);
			$mail->commit();
			$mail->send();
		}
		...
		private function executeComponentManifests() {
			...
			$this->includeCore();
			$this->fillInstructionWithUnattachedComponents();
		...
		private function fillInstructionWithSites() {
			...
			foreach ($templateList as $template) {
				$templateNameList[] = $template->getName();
			}

			$this->fillInstructionWithComponents($templateNameList);
		}

		/** Добавляет в инструкцию по обновлению узлы с непривязанными к лицензии компонентами */
		private function fillInstructionWithUnattachedComponents() {
			$this->fillInstructionWithComponents(cmsController::getInstance()->getModulesList());
		}

		/**
		 * Добавляет в инструкцию по обновлению узлы с переданными именами компонентов
		 * @param array $componentNameList список имен компонетнов
		 */
		private function fillInstructionWithComponents(array $componentNameList) {
			$instruction = $this->getUpdateInstructions();
			$xpath = new DOMXPath($instruction);
			...
			foreach ($componentNameList as $componentName) {
				...
				/** @var DOMNodeList $componentNode */
				$componentNode = $xpath->evaluate("/package/component[@name = '${componentName}']");
				...
				$componentNode = $instruction->createElement('component');
				$componentNode->setAttribute('name', $componentName);

				$packageNode->appendChild($componentNode);
			}
		...
		private function getAvailableVersions() {
			...
			$selectedVersion = $this->getSelectedVersion();
			$versionList = $this->requestVersionList();

			if (count($versionList) <= 1) {
				$selectedVersion = self::NEWEST_VERSION_ID;
			}

			$isVersionSelected = $selectedVersion !== null;

			if ($isVersionSelected) {
				$this->flushLog('Выбрана версия');
				$this->flushLog($selectedVersion);
				throw new Exception('Save version');
			}

			$this->flushLog('Необходимо выбрать версию:');
			$changeLogPattern ='%s-%d-%d';

			foreach ($versionList as $version => $date) {
				$matches = [];

				if (!preg_match('|(\d{1,})_(\d{1,})|', $version, $matches)) {
					throw new Exception(sprintf('Передана некорректная версия: "%s"', $version));
				}

				$this->flushLog(sprintf($changeLogPattern, $date, $matches[1], $matches[2], $version));
			}
		...
		/**
		 * Запрашивает список доступных версий на сервере обновлений
		 * @return array
		 * @throws Exception
		 */
		private function requestVersionList() {
			try {
				$downloader = $this->getDownloader();
				$versionListRequest = $downloader->buildUrl('get-version-list');
				$versionListResponse = $downloader->getRemoteFile($versionListRequest);
			...
			$document->loadXML($versionListResponse);
			$xpath = new DOMXPath($document);
			$versionList = [];

			/** @var DOMElement $version */
			foreach ($xpath->query('/versions/version') as $version) {
				$versionList[$version->getAttribute('name')] = $version->getAttribute('date');
			}
		...
		/**
		 * Возвращает версию системы, которую необходимо установить или до которой
		 * нужно выполнить обновление
		 * @return string|null
		 * @throws Exception
		 */
		private function getSelectedVersion() {
			if ($this->installMode) {
				return self::NEWEST_VERSION_ID;
			}

			if (INSTALLER_CLI_MODE) {
				return $this->getConfigOption('SYSTEM', 'version', self::NEWEST_VERSION_ID);
			}

			if (isset($_REQUEST['version']) || !empty($_REQUEST['version'])) {
				return (string) $_REQUEST['version'];
			}
		...
		/**
		 * Удаляет файлы инсталлятора
		 * @return bool
		 */
		private function deleteInstaller() {
			try {
				$keepInstaller = $this->getConfigOption('UPDATES', 'keep-installer', false);
			} catch (Exception $exception) {
				$keepInstaller = false;
			}
			...
			if (INSTALLER_CLI_MODE) {
				return $this->deleteFile('install.ini') && $this->deleteFile('installer.php');
			}

			return $this->deleteFile('install.php') && $this->deleteFile('__install.php');
		}
		...
		private function deleteFile($path) {
			$path = INSTALLER_CURRENT_WORKING_DIR . '/' . ltrim($path, '/');
	...
	class umiUpdateDownloader {
		...
		private $destination, $key, $host, $ip, $current_revision, $targetRevision;
		 ...
		 * @param mixed $targetRevision - Номер целевой ревизии
		 * @throws Exception
		 */
		public function __construct($destination, $key, $host, $ip, $currentRevision = umiInstallExecutor::NEWEST_VERSION_ID, $targetRevision = umiInstallExecutor::NEWEST_VERSION_ID) {
		...
		public function buildUrl($type, $params = []) {
			...
			$params['target_branch'] = $this->targetRevision;
