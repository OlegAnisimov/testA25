<?php
	...
	interface iCmsController {
		...
		/** @const array VIRTUAL_MODULE_LIST список виртуальных модулей */
		const VIRTUAL_MODULE_LIST = ['core', 'system', 'custom'];
		...
		/**
		 * Устанавливает, что контроллер для текущего запроса не найден
		 * @return $this
		 */
		public function setNotFoundState();

		/**
		 * Определяет установлен ли контроллер для текущего запроса
		 * @return bool
		 */
		...
		/**
		 * Возвращает первый доступный модуль
		 * @return string
		...
		/**
		 * Проверяет является ли модуль виртуальным
		 * @param string $moduleName имя модуля
		 * @return bool
		 */
