<?php
	...
	class csvExporter extends umiExporter {
		...
		/** @const array OPTIONED_PARTS части значения поля с типом "Составное" */
		const OPTIONED_PARTS = ['int', 'varchar', 'text', 'float'];
		...
		protected function initializeExporter() {
			$this->exporter = $this->createXmlExporter($this->sourceName, $this->getLimit());
			$this->exporter->ignoreRelationsExcept(['options', 'symlinks']);
		...
		protected function getSystemProperties(DOMElement $entity) {
			...
			$altName = $entity->getAttribute('alt-name');
			return [
				$id,
				$name,
				$typeId,
		...
		protected function loadSystemHeaders() {
			...
			$this->titleList = [
				...
				getLabel('csv-property-alt-name', 'exchange'),
			];
			$this->typeList = [
				'native',
				'native',
		...
		private function getPropertyValue(DOMElement $property) {
			switch ($this->getPropertyType($property)) {
				...
				case 'multiple_file' : {
					return $this->getMultipleFileValue($property);
				}
				case 'optioned' : {
				...
				case 'lang_id' :
				case 'lang_id_list' : {
					return $this->getLangValue($property);
				}
				case 'price_type_id' : {
					return $this->getPriceValue($property);
				}
				default : {
		...
		/**
		 * Возвращает значение свойства с типом "Ссылка на язык"
		 * или "Ссылка на список языков"
		 * @param DOMElement $property узел свойства
		 * @return string
		 */
		private function getLangValue(DOMElement $property) : string {
			$valueNode = $property->getElementsByTagName('value')->item(0);
			$langIdList = [];

			/** @var DOMElement $langNode */
			foreach ($valueNode->getElementsByTagName('lang') as $langNode) {
				$langIdList[] = $langNode->getAttribute('id');
			}

			return $this->joinMultipleValues($langIdList);
		}

		/**
		 * Возвращает значение свойства с типом "Ссылка на тип цены"
		 * @param DOMElement $property узел свойства
		 * @return string
		 */
		private function getPriceValue(DOMElement $property) : string {
			$valueNode = $property->getElementsByTagName('value')->item(0);
			$priceTypeIdList = [];

			/** @var DOMElement $priceNode */
			foreach ($valueNode->getElementsByTagName('price') as $priceNode) {
				$priceTypeIdList[] = $priceNode->getAttribute('id');
			}

			return $this->joinMultipleValues($priceTypeIdList);
		}
		...
		/**
		 * Возвращает значение свойства с типом "Набор изображений"
		 * @param DOMElement $property узел свойства
		 * @return string
		 */
		private function getMultipleImageValue(DOMElement $property) {
			return $this->getMultipleFieldValue($property, $this->getMultipleFileParts(umiImageFile::ATTRIBUTE_LIST));
		}

		/**
		 * Возвращает значение свойства с типом "Набор файлов"
		 * @param DOMElement $property узел свойства
		 * @return string
		 */
		private function getMultipleFileValue(DOMElement $property) {
			return $this->getMultipleFieldValue(
				$property,
				$this->getMultipleFileParts(umiFile::ATTRIBUTE_LIST)
			);
		}

		/**
		 * Возвращает атрибуты значения множественного поля которое содержит файлы
		 * @param string[] $fileAttributeList список атрибута файла
		 * @return array
		 */
		private function getMultipleFileParts(array $fileAttributeList) {
			array_unshift($fileAttributeList, 'path');
			return $fileAttributeList;
		}

		/**
		 * Возвращает значение свойства с множественным типом
		 * @param DOMElement $property узел свойства
		 * @param string[] $parts части значения поля
		 * @return string
		 */
		private function getMultipleFieldValue(DOMElement $property, $parts) {
			$multipleValueList = [];
			...
			foreach ($property->getElementsByTagName('value') as $valueNode) {
				...
				foreach ($parts as $partName) {
					$multipleParts[] = $this->makePart($partName, $valueNode->getAttribute($partName));
				}

				$multipleValueList[] = $this->joinMultipleParts($multipleParts);
			}

			return $this->joinMultipleValues($multipleValueList);
		}
		...
		private function getOption(DOMElement $optionNode) {
			...
			foreach (self::OPTIONED_PARTS as $partName) {
				$value = '';
		...
		private function normalizeProperties($propertyList) {
			foreach (array_keys($this->nameList) as $index) {
				if (array_key_exists($index, $propertyList)) {
					$propertyList[$index] = str_replace(
						self::VALUE_LIMITER,
