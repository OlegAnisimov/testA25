<?php
	...
	class csvSplitter extends umiImportSplitter {
		...
		/** @const int Количество частей значения поля "Набор изображений" */
		const MULTIPLE_IMAGE_PART_COUNT = 4;

		/** @const int Количество частей значения поля "Набор файлов" */
		const MULTIPLE_FILE_PART_COUNT = 3;
		 ...
		 * из файла импорта.
		 */
		protected $simpleAttributeList = ['id', 'is-active', 'is-visible', 'is-deleted', 'alt-name'];
		...
		private $imageUploadFolder = 'csv-auto-upload';
		 ...
		 */
		protected function translatePropertyList(array $propertyList, $typeId) {
			foreach ($propertyList as $key => $value) {
				$this->translateProperty($key, $value, $typeId);
			}
		}
		...
		private function normalizeRow($row) {
			...
			$row = (string) preg_replace(
				"/([^{$this->propertyDelimiter}])" . str_repeat(self::VALUE_LIMITER, 2) . '/s',
				"$1'*//*'",
				$row
			);
		 ...
		 * @param int $typeId идентификатор типа данных
		 */
		private function translateProperty($key, $value, $typeId) {
		 ...
		 */
		private function addProperty($key, $value, $typeId) {
			...
			if (!$isFilled && $objectType instanceof iUmiObjectType && !$objectType->getFieldId($name)) {
				return;
			}
			...
			if ($isMultiple) {
				$typeNode->setAttribute('multiple', 'multiple');
			}
			...
			switch ($type) {
				...
				case 'multiple_file' : {
					$this->setMultipleFileProperty($value);
					break;
				}
				case 'optioned' : {
				...
				case 'lang_id' :
				case 'lang_id_list' : {
					$this->setLangProperty($value, $isMultiple);
					break;
				}
				case 'price_type_id' : {
					$this->setPriceProperty($value, $isMultiple);
					break;
				}
				default : {
		...
		/**
		 * Устанавливает значение текущему свойству с типом данных
		 * "Ссылка на язык" или "Ссылка на список языков"
		 * @param string $value значение свойства из csv файла
		 * @param bool $isMultiple является ли поле многозначным
		 */
		private function setLangProperty(string $value, bool $isMultiple) : void {
			$valueNode = $this->document->createElement('value');
			$this->property->appendChild($valueNode);

			$langIdList = $this->extractMultiplePropertyValues($value);
			$langIdList = $isMultiple ? $langIdList : (array) getFirstValue($langIdList);

			foreach ($langIdList as $langId) {
				$langNode = $this->document->createElement('lang');
				$langNode->setAttribute('id', $langId);
				$valueNode->appendChild($langNode);
			}
		}

		/**
		 * Устанавливает значение текущему свойству с типом данных "Ссылка на тип цены"
		 * @param string $value значение свойства из csv файла
		 */
		private function setPriceProperty(string $value, bool $isMultiple) : void {
			$valueNode = $this->document->createElement('value');
			$this->property->appendChild($valueNode);

			$priceTypeIdList = $this->extractMultiplePropertyValues($value);
			$priceTypeIdList = $isMultiple ? $priceTypeIdList : (array) getFirstValue($priceTypeIdList);

			foreach ($priceTypeIdList as $priceTypeId) {
				$priceNode = $this->document->createElement('price');
				$priceNode->setAttribute('id', $priceTypeId);
				$valueNode->appendChild($priceNode);
			}
		}
		...
		/**
		 * Устанавливает значение текущему свойству с типом данных "Изображение".
		 * @param string $value значение свойства
		 * @throws umiRemoteFileGetterException
		 * @throws Exception
		 */
		private function setImageProperty($value) {
		...
		private function setMultipleImageProperty($value) {
			$this->setMultipleProperty($value, self::MULTIPLE_IMAGE_PART_COUNT);
		}

		/**
		 * Устанавливает значение текущему свойству с типом данных "Множественный файл".
		 * Свойство состоит из двух частей: 'path', 'ord'.
		 * @param string $value значение свойства
		 * @throws umiRemoteFileGetterException
		 */
		private function setMultipleFileProperty($value) {
			$this->setMultipleProperty($value, self::MULTIPLE_FILE_PART_COUNT);
		}

		/**
		 * Устанавливает значение текущему свойству с множественным типом данных
		 * @param string $content значение свойства
		 * @param int $partCount количество частей значения поля
		 * @throws umiRemoteFileGetterException
		 * @throws Exception
		 */
		private function setMultipleProperty($content, $partCount) {
			$valueList = explode($this->valueDelimiter, $content);

			foreach ($valueList as $value) {
				$partList = explode($this->partDelimiter, $value);

				if (umiCount($partList) > $partCount) {
					continue;
				}
				...
				foreach ($partList as $part) {
					list($partName, $partValue) = explode($this->subPartDelimiter, $part, 2);

					if ($partName === "path") {
						if ($this->isHttpLink($partValue)) {
							$partValue = $this->downloadImage($partValue);
						}
					}
		 ...
		 * @throws Exception
		 */
		private function downloadImage($link) {
			$directory = $this->getImageUploadDirectory();
			$image = Service::ImageUploader()->uploadByHttpLink($link, $directory);

			return ($image instanceof iUmiImageFile) ? $image->getFilePath() : $link;
		}
		 ...
		 * @return string
		 */
		private function getImageUploadDirectory() {
			$folder = mainConfiguration::getInstance()->includeParam('user-images-path');
			return $folder . '/' . $this->imageUploadFolder . '/' . date('Y-m-d') . '/';
		}

		/**
		 * Определяет является ли переданный путь http ссылкой
		 * @param string $filePath
		 * @return string
		 * @throws Exception
		 */
		private function isHttpLink($filePath) {
			return preg_match('|(https?://)([\p{Ll}0-9-_]+\.)+([\p{Ll}]+)|u', $filePath);
		}
