...
SQL;
		...
		/** @inheritDoc */
		public function isTypeRelatedToSystemSource(int $id) : bool {
			return $this->isRelatedToSystemSource($id, 'cms3_import_types');
		}

		/** @inheritDoc */
		public function setPriceTypeIdRelation($sourceId, $extId, $id) {
			$connection = ConnectionPool::getInstance()->getConnection();
			$sourceId = (int) $sourceId;
			$extId = $connection->escape($extId);
			$id = (int) $id;
			...
			$deleteSql = <<<SQL
DELETE FROM `cms3_import_offer_price_type_list` WHERE `source_id` = $sourceId AND (`new_id` = $id OR `old_id` = '{$extId}')
SQL;
			$connection->query($deleteSql);

			$insertSql = <<<SQL
INSERT INTO `cms3_import_offer_price_type_list` (`source_id`, `old_id`, `new_id`) VALUES ($sourceId, '{$extId}', $id)
SQL;
			$connection->query($insertSql);
		...
		/** @inheritDoc */
		public function getNewPriceTypeIdRelation($sourceId, $extId) {
			$connection = ConnectionPool::getInstance()
				->getConnection();
			$sourceId = (int) $sourceId;
			$extId = $connection->escape($extId);
			...
			$selectSql = <<<SQL
SELECT `new_id` FROM `cms3_import_offer_price_type_list` WHERE `old_id` = '{$extId}' AND `source_id` = $sourceId LIMIT 0,1
SQL;
			$result = $connection->queryResult($selectSql);
			$result->setFetchType(IQueryResult::FETCH_ROW);
			$id = null;
		...
		/**
		 * Определяет связана ли импортированная сущность с системным импортом,
		 * то есть была ли она создана при установке или обновлении системы.
		 * @param int $id внутренний идентификатор
		 * @param string $table имя таблицы, где хранится сущность
		 * @return bool
		 */
		private function isRelatedToSystemSource(int $id, string $table) : bool {
			$connection = ConnectionPool::getInstance()
				->getConnection();
			$sourceId = (int) $this->getSourceId('system');
			$id = (int) $id;
			$table = $connection->escape($table);
