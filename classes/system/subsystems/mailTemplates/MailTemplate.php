...
SQL;
		...
		public function parse(array $params = [], array $objectList = []) {
			$event = Service::EventPointFactory()
				->create('mailTemplatesParse', 'before')
				->setParam('mailTemplate', $this)
				->setParam('params', $params)
				->setParam('objectList', $objectList);
			$event->call();

			$mailTemplate = $event->getParam('mailTemplate');
			$params = $event->getParam('params');
			$objectList = $event->getParam('objectList');

			if (!$mailTemplate instanceof MailTemplate) {
				trigger_error('Custom mailTemplate must be of type MailTemplate, system using with default mailTemplate.', E_USER_WARNING);
				$result = (new Parser($this))->parse($params, $objectList);
			} else {
				$result = (new Parser($mailTemplate))->parse($params, $objectList);
			}
