		...
		protected function buildRelationForCollection(iCollection $collection, $relation,  array $schema) {
			...
			switch ($type) {
				...
				case iMapper::ONE_ID_TO_COLLECTION: {
					...
					foreach ($collection as $entity) {
						$dependencyList = $dependencyCollection->getListBy($referredIdField, $entity->getId());
						$dependencySubCollection = $service->mapCollection($dependencyList);
				...
				case iMapper::ONE_ENTITY_TO_COLLECTION: {
					...
					foreach ($collection as $entity) {
						...
						$dependencySubCollection = $dependencyCollection->copy()->filter([
							$idField => [
								Filter::COMPARE_TYPE_EQUALS => $attributeAccessor->accessOne($entity, $idField)
							]
						]);
