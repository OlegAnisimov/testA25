<?php
	...
	abstract class baseRestriction {
		...
		/** @var string[] SYSTEM_RESTRICTION_LIST список имен системных ограничений поля */
		const SYSTEM_RESTRICTION_LIST = ['webFormMessage'];
...
SQL;
		...
		/**
		 * Возвращает список публичных ограничений поля
		 * @return baseRestriction[]
		 * @throws databaseException
		 */
		final public static function getPublicList() {
			return array_filter(self::getList(), function (baseRestriction $restriction) {
				return !in_array($restriction->getClassName(), self::SYSTEM_RESTRICTION_LIST);
			});
		}
