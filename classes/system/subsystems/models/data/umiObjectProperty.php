<?php
	...
	abstract class umiObjectProperty extends umiEntinty implements iUmiObjectProperty {
		...
		public static function loadPropsData(array $objectIdList, $hierarchyTypeIds = false) {
			...
			self::loadFileFieldValues($objectIdList);
...
SQL;
		...
		/**
		 * todo: move it from here
		 * Загружает значения полей файлов объектов
		 * @param int[] $objectIdList список идентификаторов объектов
		 * @throws databaseException
		 * @throws Exception
		 */
		private static function loadFileFieldValues(array $objectIdList) {
			$tableName = Service::ObjectPropertyValueTableSchema()
				->getFilesTable();
			$objectsIdCondition = implode(',', $objectIdList);
			$sql = <<<SQL
SELECT `id`, `obj_id`, `field_id`, `src`, `title`, `ord`
FROM `$tableName` 
WHERE `obj_id` IN ($objectsIdCondition)
SQL;
			$result = ConnectionPool::getInstance()
			...
			foreach ($result as $row) {
				$cache[$row['obj_id']][$row['field_id']]['file_val'][] = [
					'id' => $row['id'],
					'src' => $row['src'],
					'title' => $row['title'],
					'ord' => $row['ord']
		...
		public static function clearCache() {
			umiObjectProperty::$dataCache = [];
			umiObjectProperty::$dataCacheOrder = [];
		}
