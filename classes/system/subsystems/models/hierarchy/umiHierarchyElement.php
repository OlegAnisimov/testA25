...
SQL;
		...
		private function getRightAltName($altName, $useDenseNumbering = false) {
			return umiHierarchy::getInstance()->getRightAltName($altName, $this, $useDenseNumbering);
		}
...
SQL;
		...
		private function ensureTemplate() {
			...
			if (!$umiTemplates->isExists($templateId)) {
				throw new coreException('Incorrect template id given');
			}
		...
		public function updateYML() {
			...
			if ($this->is_active && !$this->is_deleted) {
				...
				$exporter->setSerializeOption(xmlTranslator::ESCAPE_OPTION, true);
				$exporter->setSerializeOption(xmlTranslator::IGNORE_I18N, false);
				$umiDump = $exporter->execute();

				$style_file = CURRENT_WORKING_DIR . '/styles/common/xsl/export/YML.xsl';
				...
				if ($offers->length) {
					...
					foreach ($offers as $offer) {
						...
						$content .= $dom->saveXML($offer);
					}
					file_put_contents($xml, $content);
				}

				$currencies = $dom->getElementsByTagName('currencies')->item(0);
				file_put_contents($dirName . 'currencies', $dom->saveXML($currencies));
				...
				if (is_array($matches)) {
					...
					foreach ($matches as $exportId) {
						...
						file_put_contents($dirName . 'shop' . $exportId,
							...
							$companyName . '</company><url>' . $domain->getUrl() . '</url>'
						);
					}
				}
			}
