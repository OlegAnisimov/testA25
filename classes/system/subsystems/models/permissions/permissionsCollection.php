<?php
	...
	class permissionsCollection extends singleton implements iSingleton, iPermissionsCollection {
		...
		public function isAllowedModule($ownerId, $module) {
			if ($this->isVirtualModule($module)) {
				return true;
			}
...
SQL;
		...
		/**
		 * Определяет является ли модуль виртуальным
		 * @param string $module имя модуля
		 * @return bool
		 */
		private function isVirtualModule($module) {
			return cmsController::getInstance()->isVirtualModule($module);
		}
