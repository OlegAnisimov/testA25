	...
	class umiRedirectsCollection implements
		...
		public function redirectIfRequired($source, $madeByUser = false) {
			...
			do {
				...
				$sourcePath = isset($redirect) ? $redirect->getTarget() : $source;
				$redirect = $this->getRedirectByPath($sourcePath, $madeByUser);

				if ($redirect)  {
					$redirectTarget = $redirect;
				}
			} while ($redirect && $securityCounter < 20);

			if ($redirectTarget) {
				$this->redirect($redirectTarget->getTarget(), $redirectTarget->getStatus());
			}
		...
		public function init() {
			...
			$handlerFactory = Service::EventHandlerFactory();

			foreach ($this->getHandledEventsForAutoCreating() as $eventName) {
				$module = $map->get('AUTO_CREATE_REDIRECT_HANDLER_MODULE');
				$method = $map->get('AUTO_CREATE_REDIRECT_HANDLER_METHOD');
				$handlerFactory->createForModule($eventName, $module, $method);
			}
		}
		...
		protected function getUrlSuffix() {
			...
			if ($configuration->get($map->get('CONFIG_SECTION'), $map->get('CONFIG_URL_SUFFIX_ENABLE'))) {
				return $this->urlSuffix = (string) $configuration->get(
					$map->get('CONFIG_SECTION'),
					$map->get('CONFIG_URL_SUFFIX')
				);
			}
		...
		/**
		 * Возвращает редирект для адреса страницы
		 * @param string $source адрес страницы
		 * @param bool|int $madeByUser учитывать редиректы ручные/автоматические
		 * @return umiRedirect|bool
		 */
		private function getRedirectByPath($source, $madeByUser = false) {
			$connection = $this->getConnection();
			$source = $connection->escape($this->parseUri($source));
			...
			do {
				...
				if (umiCount($subRedirects) > 0) {
					...
					if ($this->isValidLink($subTarget)) {
						$subRedirect->setTarget($subTarget);
						return $subRedirect;
					}
				}
