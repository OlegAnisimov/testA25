<?php
	...
	class systemInfo implements iSystemInfo {
		...
		/** @var string CHANGE_LOG_FILE_NAME имя файла с журналом изменений */
		const CHANGE_LOG_FILE_NAME = 'CHANGELOG.md';
		...
		public function getInfo($option = 1) {
			...
			if (($option & self::CHANGE_LOG) === self::CHANGE_LOG) {
				try {
					$out['change_log'] = $this->getChangeLog();
		...
		/**
		 * Возвращает содержимое журнала изменений
		 * @return string
		 * @throws coreException
		 */
		private function getChangeLog() : string {
			$logsDirectoryPath = $this->config->includeParam('sys-log-path');
			$changeLogPath = $logsDirectoryPath . '/' . self::CHANGE_LOG_FILE_NAME;
			$changeLog = $this->fileFactory->create($changeLogPath);

			if (!$changeLog->isExists()) {
				throw new coreException(__METHOD__ . ': change log not found');
			}

			if (!$changeLog->isReadable()) {
				throw new coreException(__METHOD__ . ': change log reading not permitted');
			}

			return (string) $changeLog->getContent();
		}
