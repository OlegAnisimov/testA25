<?php
	...
	use UmiCms\Service;
	use UmiCms\System\Selector\Order\iAttribute as iOrderAttribute;
	use UmiCms\System\Selector\Order\Attribute\iField as iOrderField;
	use UmiCms\System\Selector\Order\Attribute\iProperty as iOrderProperty;
	use UmiCms\System\Selector\Order\Attribute\Property\iGlobalOrd as iOrderPropertyGlobalOrd;
	...
	class selectorExecutor {
		...
		/** @var int[] $hierarchyElementCandidateIdList список идентификаторов страниц, которые могут оказаться результирующими */
		protected $hierarchyElementCandidateIdList = [];
		...
		public function result() {
			...
			try {
				$this->prepareOrderBefore();
				$result = $connection->queryResult($sql);
			} catch (databaseException $exception) {
				throw $exception;
			} finally {
				$this->prepareOrderAfter();
			}
		...
		public function length() {
			...
			$isNeedRows = umiCount($this->selector->groupSysProps) || umiCount($this->selector->groupFieldProps);
		...
		/**
		 * Возвращает список идентификаторов страниц, которые могут оказаться результирующими
		 * @return array|int[]
		 */
		public function getHierarchyElementCandidateIdList() : array {
			return $this->hierarchyElementCandidateIdList;
		}

		/**
		 * Возвращает список идентификаторов объектных типов, по которым идет выборка
		 * @return array|int[]
		 */
		public function getObjectTypeIdList() : array {
			return $this->objectTypeIds;
		}

		/**
		 * Возвращает список идентификаторов иерархических (базовых) типов, по которым идет выборка
		 * @return array|int[]
		 */
		public function getHierarchyTypeIdList() : array {
			return $this->hierarchyTypeIds;
		}

		/**
		 * Возвращает опции выборки
		 * @return selector
		 */
		...
		protected function requireSysProp($propName) {
			...
			if ($this->selector->mode == 'pages') {
				...
				$propTable['global_ord'] = ['g.parent_ord, g.ord', 'table' => ['g', 'cms3_hierarchy_global_ord']];
				$propTable['id'] = ['h.id'];
			} else {
		...
		protected function analyzeFields() {
			...
			foreach ($selectorOrderFields as $orderField) {
				if (!$orderField instanceof iOrderField) {
					continue;
				}
...
SQL;
		...
		protected function buildLeftJoins() {
			...
			/** @var selectorGroupFieldProp|iOrderField|selectorWhereFieldProp $data_join */
...
SQL;
		...
		protected function buildWhere() {
			...
			if ($this->selector->mode == 'pages') {
				foreach ($this->selector->orderSysProps as $orderSysProp) {
					if ($orderSysProp instanceof iOrderPropertyGlobalOrd) {
						$conditions[] = 'g.id = h.id';
						break;
					}
				}
			}
		...
		protected function buildWhereValue($fieldId) {
			...
			foreach ($current as $where) {
				...
				if (($fieldDataType === 'file' || $fieldDataType === 'img_file') && $column === 'src') {
		...
		protected function buildOrder() {
			...
			foreach ($this->selector->orderFieldProps as $order) {
				if (!$order instanceof iOrderField) {
					continue;
				}
				...
				foreach ($order->getFieldIdList() as $fieldId) {
					...
					if ($column == 'file' || 
						$column == 'img_file' || 
						$column == 'multiple_file' || 
						$column == 'multiple_image') {
						$column = 'src';
					}
			...
			foreach ($this->selector->orderSysProps as $order) {
				if (!$order instanceof iOrderProperty) {
					continue;
				}
...
SQL;
			...
			$this->hierarchyElementCandidateIdList = $pagesIds;
		...
		protected static function getFieldColumn($fieldId) {
			...
			switch ($field->getDataType()) {
				...
				case 'multiple_file':
					return $cache[$fieldId] = 'multiple_file';
				...
				case 'lang_id':
				case 'lang_id_list':
					return $cache[$fieldId] = 'lang_id';

				case 'price_type_id':
					return $cache[$fieldId] = 'price_type_id';
...
SQL;
		...
		/**
		 * Вызывает обработку сортировок до результирующей выборки
		 * @throws selectorException
		 */
		private function prepareOrderBefore() : void {
			$this->prepareOrder(function(iOrderAttribute $order) {
				$order->beforeQuery($this);
			});
		}

		/**
		 * Вызывает обработку сортировок после результирующей выборки
		 * @throws selectorException
		 */
		private function prepareOrderAfter() : void {
			$this->prepareOrder(function(iOrderAttribute $order) {
				$order->afterQuery($this);
			});
		}

		/**
		 * Вызывает обработку сортировок
		 * @param callable $method метод обработки
		 * @throws selectorException
		 */
		private function prepareOrder(callable $method) : void {
			foreach ($this->selector->orderSysProps as $order) {
				if (!$order instanceof iOrderProperty) {
					continue;
				}
				call_user_func($method, $order);
			}
