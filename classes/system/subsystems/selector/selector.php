<?php
	...
	use UmiCms\Service;
	use UmiCms\System\Selector\Order\iFactory as iOrderFactory;
	...
	class selector implements IteratorAggregate {
		...
		/** @var string[] Системные свойства страниц при сортировке */
		protected static $sysPagesOrderFields = ['name', 'ord', 'rand', 'updatetime', 'id', 'global_ord'];
		...
		/** @var string[] Системные свойства объектов при сортировке */
		protected static $sysObjectsOrderFields = ['name', 'ord', 'rand', 'updatetime', 'id'];
		...
		/** @var iOrderField[] Системные свойства, по которым идет сортировка */
		...
		/** @var iOrderProperty[] Свойства, по которым идет сортировка */
		...
		public function where($fieldName) {
			...
			if (in_array($fieldName, ($this->mode == 'pages')
				? self::$sysPagesWhereFields
				: self::$sysObjectsWhereFields)) {
			...
			if (empty($fieldIdList)) {
				throw new selectorException(
					__METHOD__ . ": Field \"{$fieldName}\" is not presented in selected object types"
				);
			}
			...
			return $this->whereFieldProps[] =
				new selectorWhereFieldProp($fieldIdList, $this->option('search-in-related-object')->value);
		}

		/**
		 * Сортировать результат по полю
		 * @param string $fieldName Имя поля для сортировки
		 * @return iOrderField|iOrderProperty
		 * @throws selectorException если поле не существует
		 */
		public function order($fieldName) {
			$this->throwIfAlreadyExecuted();
			/** @var iOrderFactory $factory */
			$factory = Service::get(iOrderFactory::SERVICE_NAME);

			if (in_array($fieldName, $this->getSysOrderFields())) {
				return $this->orderSysProps[] = $factory->createForProperty($fieldName);
			}
			...
			if (empty($fieldIdList)) {
				throw new selectorException(
					__METHOD__ . ": Field \"{$fieldName}\" is not presented in selected objects types"
				);
			}

			return $this->orderFieldProps[] = $factory->createForField($fieldIdList);
		}
		...
		public function group($fieldName) {
			...
			if (in_array($fieldName, ($this->mode == 'pages')
				? self::$sysPagesGroupFields
				: self::$sysObjectsGroupFields)) {
		 ...
		 * @param int|string $limit нужное число результатов или ключевое слово,
		...
		/**
		 * Отключает ограничение количества результатов выборки
		 * @return $this
		 */
		...
		/**
		 * Возвращает системные свойства при сортировке
		 * @return array|string[]
		 */
		protected function getSysOrderFields() : array {
			return ($this->mode == 'pages') ? self::$sysPagesOrderFields : self::$sysObjectsOrderFields;
		}
