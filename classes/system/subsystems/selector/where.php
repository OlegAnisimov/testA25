			...
			if (isset($this->fieldIdList)) {
				...
				if ($field->getDataType() === 'relation' && $isString && $relatedMode && $field->getGuideId()) {
					...
					$usersType = selector::get('object-type')
						->name('users', 'user');
					$usersTypeId = ($usersType instanceof iUmiObjectType) ? $usersType->getId() : null;
					$correctTypeIdList = [
						$guideId
					];
					 ...
					 */
					if ($field->getName() == 'customer_id' && $field->getGuideId() == $usersTypeId) {
						/** @var iUmiObjectType $customerType */
						$customerType = selector::get('object-type')
							->name('emarket', 'customer');
						$customerTypeId = ($customerType instanceof iUmiObjectType) ? $customerType->getId() : null;

						$sel->types('object-type')->id($customerTypeId);

						$correctTypeIdList[] = $customerTypeId;
					}

					if (is_numeric($value) && $obj instanceof iUmiObject && in_array($obj->getTypeId(), $correctTypeIdList)) {
