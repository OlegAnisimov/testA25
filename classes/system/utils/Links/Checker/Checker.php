<?php
	...
	class Checker implements iChecker, \iUmiService, \iUmiRegistryInjector, Injectors\iLinksCollection {
		...
		/** @var $domainDetector iDetector определитель домена */
		...
		const HOST_WITH_PROTOCOL_PATTERN = '/(https?:\/\/)([\p{Ll}0-9-_]+\.){1,}([\p{Ll}]+){1}/u';

		/** @const string HOST_PATTERN регулярное выражение для получения хоста */
		const HOST_PATTERN = '/([\p{Ll}0-9-_]+\.){1,}([\p{Ll}]+){1}/u';

		/**
		 * @const string PROTOCOL_RELATIVE_HOST_PATTERN
		 * регулярное выражение для получения хоста с относительным протоколом
		 */
		const PROTOCOL_RELATIVE_HOST_PATTERN = '/^(\/\/)([\p{Ll}0-9-_]+\.){1,}([\p{Ll}]+){1}/u';
		...
		private function getAbsoluteLink(iEntity $entity) {
			...
			if ($this->isProtocolRelativeUrl($linkAddress)) {
				return $this->domainDetector->detect()->getProtocol() . ":$linkAddress";
			}
		...
		private function isAbsoluteUrl($string) {
			return startsWith($string, self::URL_PROTOCOL_START_SYMBOLS);
		}

		/**
		 * Содержит ли адрес ссылки относительный протокол
		 * @param string $address адрес ссылки
		 * @return bool
		 */
		private function isProtocolRelativeUrl($address) {
			return (bool) preg_match(self::PROTOCOL_RELATIVE_HOST_PATTERN, $address);
		}
		...
		private function isUrlPath($string) {
			return startsWith($string, self::URI_SEPARATOR);
		}
		...
		private function isUrlQuery($string) {
			return startsWith($string, self::URL_QUERY_START_SYMBOL);
		}
		...
		private function isUrlFragment($string) {
			return startsWith($string, self::URL_FRAGMENT_START_SYMBOL);
		}
		...
		private function isUrlBroken($url) {
			return !$this->isSpecificUrlSchema($url) && ($this->requestUrlStatus($url) !== self::OK_HTTP_STATUS_CODE);
		}

		/**
		 * Содержит ли адрес специфичную URL-схему (mailto, tel)
		 * @param string $url адрес страницы сайта
		 * @return bool
		 */
		private function isSpecificUrlSchema($url) {
