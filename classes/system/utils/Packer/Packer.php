<?php
	...
	class Packer {
		...
		private function packComponent() {
			...
			$this->packInstallOnlyFiles();
			$this->packRegularFiles();

			if (file_exists('./config.distr.ini')) {
				$this->addFileToArchive(new SplFileInfo('./config.distr.ini'), $this->getArchive());
			}
		 ...
		 * @param array $files пути до файлов
		 */
		private function addInstallOnlyFiles(array $files) {
			$destination = $this->getDestinationDir();
			$this->exporter->setDestination($destination);
			$this->exporter->addInstallOnlyFiles($files);
		}
		 ...
		 * @throws RuntimeException в случае если не задана директория для выходных файлов
		 */
		private function packRegularFiles() {
			$filePathList = $this->getConfig('files');
			$filesInArchive = $this->putFilesToArchive($filePathList);

			if (count($filesInArchive) > 0) {
				$this->addFiles($filesInArchive);
			}
		}

		/**
		 * Упаковывает файлы, которые будут использованы только для первоначальной установки
		 * В конфигурационном файле необходимо указать массив путей до файлов
		 * относительно корневой директории системы.
		 * @throws RuntimeException в случае если не задана директория для выходных файлов
		 */
		private function packInstallOnlyFiles() {
			$filePathList = $this->getConfig('install_only_files');
			$filesInArchive = $this->putFilesToArchive($filePathList);

			if (count($filesInArchive) > 0) {
				$this->addInstallOnlyFiles($filesInArchive);
			}
		}

		/**
		 * Собирает файлы в архив
		 * @param array|mixed $filePathList список путей до файлов
		 * @return array
		 */
		private function putFilesToArchive($filePathList) {
		...
		private function getRegistryList($component = 'core', $parentPath = '//', $recursive = true) {
			if ($component == 'core' && startsWith($parentPath, 'modules')) {
				return [];
			}
		...
		private function addTypes() {
			...
			foreach ($typeIdList as $index => $typeId) {
				...
				$typeId = $typeCollection->getTypeIdByGUID($typeId);

				if (is_numeric($typeId)) {
					$typeIdList[$index] = $typeId;
				}
			}
		...
		private function addObjects() {
			...
			foreach ($objectIdList as $index => $objectId) {
				if (!is_numeric($objectId) && is_string($objectId)) {
					$objectIdList[$index] = $objectCollection->getObjectIdByGUID($objectId);
				}
			}
		...
		private function getArchive() {
			return new PharData($this->getPackagePath());
		}

		/** @return string */
		public function getPackagePath() {
			return "{$this->getDestinationDir()}/{$this->getConfig('package')}.tar";
		}
	}
