<?php
	 ...
	 *
	 */
	class BrowserDetect implements iDetector {
		...
		const BROWSER_YANDEXBOT = 'YandexBot'; // https://yandex.ru/support/webmaster/robot-workings/check-yandex-robots.html

		const BROWSER_ALEXABOT = 'AlexaBot';
		...
		public function __construct(iEngineFactory $cacheFactory) {
			$this->cacheStorage = $cacheFactory->create();
		}
		...
		public function isBrowser($browserName) {
			return (strcasecmp($this->_browser_name, trim($browserName)) == 0);
		}
		...
		public function isChromeFrame() {
			return (mb_strpos($this->_agent, 'chromeframe') !== false);
		}
		...
		protected function determine() {
			$key = md5(mb_strtolower(trim($this->_agent)));
			$params = $this->loadFromCache($key);

			if (empty($params)) {
				$this->checkPlatform();
				$this->checkBrowsers();
				$this->checkForAol();
				$this->checkMobileAgents();
				$data = [
					'_browser_name' => $this->getBrowser(),
					'_version' => $this->getVersion(),
					'_platform' => $this->getPlatform(),
					'_os' => $this->_os,
					'_is_aol' => $this->isAol(),
					'_is_mobile' => $this->isMobile(),
					'_is_tablet' => $this->isTablet(),
					'_is_robot' => $this->isRobot(),
					'_is_facebook' => $this->isFacebook(),
					'_aol_version' => $this->getAolVersion()
				];
		...
		protected function checkForAol() {
			...
			if (stripos($this->_agent, 'aol') !== false) {
				$version = explode(' ', stristr($this->_agent, 'AOL'));
				if (isset($version[1])) {
					$this->setAol(true);
		...
		/**
		 * Определяет зашел ли пользователь с мобильного устройства
		 * @return bool
		 */
		protected function checkMobileAgents() : bool {
			if ($this->isMobile() || $this->isTablet()) {
				return true;
			}

			$pattern = '/(Android|webOS|iPhone|iPad|iPod|BlackBerry|BB|PlayBook|IEMobile|Windows Phone|Kindle|Silk|Opera Mini)/';
			$isMobile =  (bool) preg_match($pattern, $this->_agent);
		...
		protected function checkBrowserYandexBot() {
			...
			foreach ($yandexAgentNamesList as $agentName) {
				if (stripos($this->_agent, $agentName) !== false) {
					$aresult = explode('/', stristr($this->_agent, $agentName));
					if (isset($aresult[1])) {
		...
		/**
		 * Determine if the browser is the AlexaBot or not
		 * @return boolean True if the browser is the AlexaBot otherwise false
		 */
		protected function checkBrowserAlexaBot() {
			if (stripos($this->_agent, 'ia_archiver') !== false) {
				$this->_browser_name = self::BROWSER_ALEXABOT;
				$this->setRobot(true);
				return true;
			}
			...
			if (stripos($this->_agent, 'msie') !== false && stripos($this->_agent, 'opera') === false) {
				...
				if (stripos($this->_agent, 'msnb') !== false) {
					...
					if (isset($result[1])) {
						...
						$this->setVersion(str_replace(['(', ')', ';'], '', $result[1]));
						return true;
					}
				}
		...
		protected function checkBrowserNokia() {
			if (preg_match("/Nokia([^\/]+)\/([^ SP]+)/i", $this->_agent, $matches)) {
				...
				if (stripos($this->_agent, 'Series60') !== false || mb_strpos($this->_agent, 'S60') !== false) {
		...
		protected function checkBrowserMozilla() {
			if (stripos($this->_agent, 'mozilla') !== false && preg_match('/rv:[0-9].[0-9][a-b]?/i', $this->_agent) &&
			...
			if (stripos($this->_agent, 'mozilla') !== false && preg_match('/rv:[0-9]\.[0-9]/i', $this->_agent) &&
				stripos($this->_agent, 'netscape') === false) {
				$version = explode('', stristr($this->_agent, 'rv:'));
				$this->setVersion(str_replace('rv:', '', $version[0]));
				$this->setBrowser(self::BROWSER_MOZILLA);
			...
			if (stripos($this->_agent, 'mozilla') !== false && preg_match('/mozilla\/([^ ]*)/i', $this->_agent, $matches) &&
				stripos($this->_agent, 'netscape') === false) {
				$this->setVersion($matches[1]);
				$this->setBrowser(self::BROWSER_MOZILLA);
				return true;
		...
		public static function getInstance() {
			return Service::BrowserDetector();
		}

		/**
		 * Загружает параметры браузера из кеша
		 * @param string $key ключ
		 * @return array
		 */
		private function loadFromCache($key) {
			$data = $this->getCacheStorage()
				->loadRawData($key);
			$encodedData = json_decode($data, true);
			return is_array($encodedData) ? $encodedData : [];
		}

		/**
		 * Сохраняет параметры браузера в кеш
		 * @param string $key ключ
		 * @param array $params параметры
		 * @return bool
		 */
		private function saveToCache($key, array $params) {
			return $this->getCacheStorage()
				->saveRawData($key, json_encode($params), strtotime('+2 year'));
		}
