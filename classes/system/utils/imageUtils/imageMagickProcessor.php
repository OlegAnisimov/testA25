	...
	class imageMagickProcessor implements iImageProcessor {
		...
		public function optimize($imagePath, $quality = 75) {
			...
			switch ($mimeType) {
				...
				default: {
					$quality = $this->getImageCompressionQuality($file, getImageQualityLevel($quality));
		...
		/**
		 * Возвращает качество сжатия изображения
		 * @param Imagick $image изображение
		 * @param int $quality необходимое качество
		 * @return int
		 */
		private function getImageCompressionQuality(Imagick $image, $quality) {
			return (int) $image->getImageCompressionQuality() * $quality / 100;
		}
	}
