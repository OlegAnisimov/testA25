<?php
	...
	class loginzaUserProfile {
		...
		/**
		 * Возвращает название сервиса, через который выполнен вход
		 * @return string
		 */
		public function genProviderName() : string {
			$provider = $this->genProvider();
			$provider_url = parse_url($provider);
			return str_ireplace('www.', '', $provider_url['host']);
		}
		...
		public function getFname() {
			if (!empty($this->profile->name->first_name)) {
				return $this->profile->name->first_name;
			}
		...
		public function getLname() {
			if (!empty($this->profile->name->last_name)) {
				return $this->profile->name->last_name;
			}
		...
		public function genFullName() {
			...
			if (!empty($this->profile->name->first_name) || !empty($this->profile->name->last_name)) {
		 ...
		 * @param unknown_type $char_list Список наборов символов, используемых для генерации, через запятую.
		 * Например: a-z,0-9,~
		 * @return unknown
		 */
		public function genRandomPassword($len = 6, $char_list = 'a-z,0-9') {
			...
			$chars['A-Z'] = mb_strtoupper($chars['a-z']);
			...
			for ($i = 0; $i < $len; $i++) {
				$password .= $charset[mt_rand(0, mb_strlen($charset) - 1)];
			}
		...
		private function normalize($string, $delimer = '-') {
			...
			return mb_substr(trim(preg_replace('/[^\w]+/i', $delimer, $string), $delimer), 0, 40);
		}
	}
