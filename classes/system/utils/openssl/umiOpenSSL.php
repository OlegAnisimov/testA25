<?php
	...
	class umiOpenSSL implements iUmiOpenSSL {
		...
		public function loadSettingsFromRegedit() {
			...
			$privateKeyFilePath = CURRENT_WORKING_DIR . $regedit->get('//settings/openssl_private_key');
			$publicKeyFilePath = CURRENT_WORKING_DIR . $regedit->get('//settings/openssl_public_key');
			$bResult = false;

			try {
				$bResult =
					($this->setPrivateKeyFilePath($privateKeyFilePath) && $this->setPublicKeyFilePath($publicKeyFilePath));
		...
		public function supplyDefaultKeyFiles() {
			...
			if (!$bResult) {
				...
				*/
				$sSslDir = mainConfiguration::getInstance()
					->includeParam('open-ssl-path');
				@mkdir($sSslDir);
				$oSslDir = new umiDirectory($sSslDir);
				//
				if ($oSslDir instanceof umiDirectory && !$oSslDir->getIsBroken()) {
					$bCertOk = false;
					$sCertFile = $sSslDir . '/' . uniqid('');
					$bPrivateOk = false;
					$sPrivateFile = $sSslDir . '/' . uniqid('');
					$bPublicOk = false;
					$sPublicFile = $sSslDir . '/' . uniqid('');
					...
					$privateFile = Service::FileFactory()->create($sPrivateFile);
					$publicFile = Service::FileFactory()->create($sPublicFile);

					// пишем в реестр
					if ($bPublicOk) {
						$regedit = Service::Registry();
						$bPrivateSetted = $regedit->set('//settings/openssl_private_key', $privateFile->getFilePath(true));
						$bPublicSetted = $regedit->set('//settings/openssl_public_key', $publicFile->getFilePath(true));
