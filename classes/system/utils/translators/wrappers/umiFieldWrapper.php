	...
	class umiFieldWrapper extends translatorWrapper {
		...
		public function translateData(iUmiField $field) {
			$result = [
				...
				'attribute:title' => $field->getTitle($this->getOption(xmlTranslator::IGNORE_I18N)),
			...
			if ($field->getTip()) {
				$result['tip'] = $field->getTip($this->getOption(xmlTranslator::IGNORE_I18N));
			}
			...
			if ($field->getIsSystem()) {
				$result['attribute:system'] = 'system';
			}
