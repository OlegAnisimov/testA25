	...
	class umiFieldsGroupWrapper extends translatorWrapper {
		...
		public function translateProperties(iUmiFieldsGroup $group, iUmiObject $object) {
			$result = [
				...
				'title' => $group->getTitle($this->getOption(xmlTranslator::IGNORE_I18N)),
				'tip' => $group->getTip($this->getOption(xmlTranslator::IGNORE_I18N)),
				'nodes:property' => []
			];
		...
		protected function translateData(iUmiFieldsGroup $group) {
			$result = [
				'attribute:id' => $group->getId(),
				'attribute:name' => $group->getName(),
				'attribute:title' => $group->getTitle($this->getOption(xmlTranslator::IGNORE_I18N)),
				'nodes:field' => $group->getFields()
			];

			if ($group->getTip()) {
				$result['tip'] = $group->getTip($this->getOption(xmlTranslator::IGNORE_I18N));
			}
		...
		private function isValidFieldName($name) {
			...
			if (is_array($nameBlackList) && in_array($name, $nameBlackList)) {
				return false;
			}
