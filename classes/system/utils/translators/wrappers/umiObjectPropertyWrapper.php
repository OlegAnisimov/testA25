<?php
	...
	class umiObjectPropertyWrapper extends translatorWrapper {
		...
		private $privateProperties = ['user_dock', 'user_settings_data', 'activate_code', 'auth_token'];
		...
		protected function translateData(iUmiObjectProperty $property) {
			...
			$result = [
				...
				'@name' => $field->getName(),
				'@type' => $dataType,
				'title' => $field->getTitle($this->getOption(xmlTranslator::IGNORE_I18N)),
		...
		private function translatePropertyValue(iUmiField $field, iUmiObjectProperty $property, $value) {
			...
			switch ($field->getDataType()) {
				...
				case 'lang_id': {
					return $this->translateLangIdValue($value);
				}

				case 'lang_id_list' : {
					return $this->translateLangIdListValue($value);
				}

				case 'price_type_id': {
					return $this->translatePriceTypeIdValue($value);
				}

				case 'offer_id' : {
					return $this->translateOfferIdValue($value);
				}

				case 'offer_id_list' : {
					return $this->translateOfferIdListValue($value);
				}
		...
		/**
		 * Преобразует значение поля типа "Ссылка на язык" в массив с разметкой для последующей сериализации в xml
		 * @param int $langId значение поля типа "Ссылка на язык"
		 * @return array
		 * @throws Exception
		 */
		private function translateLangIdValue(?int $langId) : array {
			return [
				'lang' => Service::LanguageCollection()
					->getLang($langId)
			];
		}

		/**
		 * Преобразует значение поля типа "Ссылка на список языков" в массив с разметкой
		 * для последующей сериализации в xml
		 * @param int[] $langIdList значение поля типа "Ссылка на список языков"
		 * @return array
		 * @throws Exception
		 */
		private function translateLangIdListValue(array $langIdList) : array {
			$langIdList = (array) $langIdList;

			$result = [
				'nodes:lang' => []
			];

			$langCollection = Service::LanguageCollection();

			foreach ($langIdList as $langId) {
				$result['nodes:lang'][] = $langCollection->getLang($langId);
			}
		...
		/**
		 * Преобразует значение поля типа "Ссылка на тип цены" в массив с разметкой для последующей сериализации в xml
		 * @param int $priceTypeId значение поля типа "Ссылка на тип цены"
		 * @return array
		 * @throws Exception
		 */
		private function translatePriceTypeIdValue(?int $priceTypeId) : array {
			return [
				'price' => Service::TradeOfferPriceTypeFacade()
					->get($priceTypeId)
			];
		}

		/**
		 * Преобразует значение поля типа "Ссылка на торговое предложение" в массив с разметкой
		 * для последующей сериализации в xml
		 * @param int $offerId значение поля типа "Ссылка на торговое предложение"
		 * @return array
		 * @throws ErrorException
		 * @throws databaseException
		 */
		private function translateOfferIdValue($offerId) {
			return [
		...
		/**
		 * Преобразует значение поля типа "Ссылка на список торговых предложений" в массив с разметкой
		 * для последующей сериализации в xml
		 * @param int[] $offerIdList значение поля типа "Ссылка на список торговых предложений"
		 * @return array
		 * @throws ErrorException
		 ...
		 * @throws Exception
		 */
		private function translateOfferIdListValue($offerIdList) {
			$offerIdList = (array) $offerIdList;
			$tradeOfferFacade = Service::TradeOfferFacade();
			$offerCollection = $tradeOfferFacade->mapCollection($tradeOfferFacade->getList($offerIdList));
			$tradeOfferFacade->loadRelations($offerCollection);
			$offerList = $offerCollection->getList();

			if (Service::Request()->isSite()) {
				$offerList = array_filter($offerList, function(iOffer $offer) {
					return $offer->isActive();
				});
			}

			$result = [
				'attribute:count' => count($offerList),
				'nodes:offer' => []
			];

			foreach ($offerList as $offer) {
				$result['nodes:offer'][] = $offer;
			}
