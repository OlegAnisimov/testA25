	...
	this.runStat = function (stat, inst, isNextNeeded, flag) {
		...
		if (flag == 'one' || flag == true) {
			jQuery.ajax({
				...
				beforeSend: function() {
					jQuery(".stat-value", stat).html('<img src="/styles/skins/modern/design/img/table/loading.gif" />');
		...
		if (flag == 'all' || flag == true) {
			jQuery.ajax({
				...
				beforeSend: function() {
					jQuery(".stat-value-all", stat).html('<img src="/styles/skins/modern/design/img/table/loading.gif" />');
				},
				success: function(data) {
					if (curInst != inst) return;
					if (data.result.status) {
	...
	this.topPopular = function(elem, sort) {
		...
		jQuery.ajax({
			...
			beforeSend: function() {
				...
				jQuery(jQuery(elem).find('tbody')).append('<tr><td colspan="4" style="text-align:center"><img src="/styles/skins/modern/design/img/table/loading.gif" /></td></tr>');
			},
			success: function(data) {
				jQuery(elem).find('tbody').find('tr').remove();
