...
<xsl:stylesheet version="1.0"
	...
	<!-- Изменять ли поле h1 товара или раздела каталога -->
	<xsl:variable name="isChangeCatalogItemH1" select="number($settings//item[@key='is_change_catalog_item_h1_from_cml'])" />

	<!-- Изменять ли поле title товара или раздела каталога -->
	<xsl:variable name="isChangeCatalogItemTitle" select="number($settings//item[@key='is_change_catalog_item_title_from_cml'])" />

	<!-- Список ставок налога НДС -->
	<xsl:variable name="tax_rates" select="$settings/taxes" />

	<!-- Идентификатор каталога из 1С -->
	<xsl:variable name="catalog-id" select="/КоммерческаяИнформация/Каталог/Ид" />

	<!-- Дата формирования файла импорта -->
	<xsl:variable name="generate-time" select="КоммерческаяИнформация/@ДатаФормирования" />
	...
	<xsl:template match="/">
		<umidump xmlns:xlink="http://www.w3.org/TR/xlink" version="2.0">
			<meta>
				...
				<generate-time>
					<timestamp><xsl:value-of select="php:function('strtotime', string($generate-time))"/></timestamp>
				</generate-time>
			</meta>
			...
			<properties>
				...
				<group name="catalog_stores_props" title="Склады">
					<property name="common_quantity" title="Общее количество на складах" type="float" is-public="1" allow-runtime-add="1">
						...
						<value>
							<xsl:choose>
								<xsl:when test="not(Количество)">0</xsl:when>
								<xsl:otherwise>
	...
	<xsl:template match="Товары/Товар">
		...
		<page id="{Ид}" parentId="{$group_id}" type-id="{$group_id}">
			...
			<properties>
				...
				<xsl:apply-templates select="." mode="tax-rates"/>
	...
	<!-- Шаблон ставок налога товара -->
	<xsl:template match="Товар" mode="tax-rates">
		<group name="cenovye_svojstva">
			<title>i18n::fields-group-cenovye_svojstva</title>
			<xsl:variable name="tax_rate">
				<xsl:choose>
					<xsl:when test="СтавкиНалогов/СтавкаНалога/Наименование/text() = 'НДС'">
						<xsl:value-of select="СтавкиНалогов/СтавкаНалога/Ставка[preceding-sibling::Наименование/text() = 'НДС']"/>
					</xsl:when>
					<xsl:otherwise>
			...
			<property name="tax_rate_id" type="relation">
				<title>i18n::field-tax-rate-id</title>
				<value>
					<item>
						<xsl:attribute name="name">
							<xsl:value-of select="$tax_rates/rate[@value = $tax_rate]/@name"/>
						</xsl:attribute>
					</item>
				</value>
			</property>
	...
	<!-- Шаблон выгрузки для группы полей "common"	-->
	<xsl:template name="common-group">
		<xsl:param name="name"/>
		...
		<group name="common">
			...
			<property name="title" type="string">
				...
				<xsl:choose>
					<xsl:when test="$isChangeCatalogItemTitle = 1">
			...
			<property name="h1" type="string">
				...
				<xsl:choose>
					<xsl:when test="$isChangeCatalogItemH1 = 1">
						<value><xsl:value-of select="$name" /></value>
					</xsl:when>
					<xsl:otherwise>
						<default-value><xsl:value-of select="$name" /></default-value>
	...
	<xsl:include href="udata://core/includeExchangeXsl/import/commerceML2" />
