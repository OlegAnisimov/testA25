...
<xsl:stylesheet version="1.0"
	...
	<!-- Шаблон полей "Способ оплаты" и "Способ доставки" -->
	<xsl:template match="field[@name = 'payment_id' or @name='delivery_id']" mode="form-modify">
		<div class="col-md-6 order-relation clearfix default-empty-validation" id="{generate-id()}" umi:type="{@type-id}" umi:empty="empty">
			<div class="title-edit">
				<span class="label">
					<acronym title="{@tip}">
						<xsl:apply-templates select="." mode="sys-tips" />
	...
	<!-- Шаблон поля "Статус оплаты" со значением "Ожидает подтверждения" -->
	<xsl:template match="field[@name = 'payment_status_id']" mode="form-modify">
		<div class="col-md-6 relation clearfix default-empty-validation" id="{generate-id()}" umi:type="{@type-id}">
			...
			<xsl:variable name="payment.id" select="../field[@name = 'payment_id']/values/item/@id"/>
			<xsl:choose>
				<!-- payment status waiting confirmation -->
				<xsl:when test="./values/item[@guid = 'emarket-order-payment-status-waiting']">
					<a id="payment-approve" href="#" class="btn color-blue btn-small" data-payment-id="{$payment.id}">
						&label-payment-approve;
					</a>
					<a id="payment-cancel" href="#" class="btn color-blue btn-small" data-payment-id="{$payment.id}">
						&label-payment-cancel;
					</a>
				</xsl:when>
				<!-- payment status accepted -->
				<xsl:when test="./values/item[@guid = 'emarket-orderpaymentstatus-27383']">
					<a id="payment-refund" href="#" class="btn color-blue btn-small" data-payment-id="{$payment.id}">
			...
			<script type="text/javascript" src="/styles/skins/modern/data/modules/emarket/js/PaymentManager.js"></script>
		</div>
	</xsl:template>
				...
				<div class="layout">
					<div class="column">
						<div class="row">
							...
							<xsl:apply-templates select="field[@name = 'yandex_map_api_key']" mode="form-modify" />
							...
							<xsl:apply-templates select="field[@name = 'include_fees']" mode="form-modify" />
							<xsl:apply-templates select="field[@name = 'disabled']" mode="form-modify" />
	...
	<xsl:template match="field[@type = 'password']" mode="form-modify">
		<div class="col-md-6 default-empty-validation">
			...
			<span>
				...
				<input type="password" name="no-hash" value="true" hidden="hidden"/>
			</span>
		</div>
	</xsl:template>
				...
				<template id="layoutsTpl">
					<![CDATA[
						...
						<div id="asps_form_wrapper">
							<img src="/styles/skins/modern/design/img/loading_bar.gif" />
						</div>
