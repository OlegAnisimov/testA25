...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="/result[@method = 'order_edit']/data/object" mode="form-modify">
		...
		<xsl:if test="$customer-id">
			<xsl:call-template name="userOrdersList">
				<xsl:with-param name="customer-id" select="$customer-id"/>
			</xsl:call-template>
		</xsl:if>
		...
		<script type="text/javascript" src="/styles/skins/modern/data/modules/emarket/js/orderEdit.js?{$system-build}"/>
	</xsl:template>
	...
	<xsl:template name="userOrdersList">
		<xsl:param name="customer-id"/>
						...
						<div class="row">
							...
							<xsl:call-template name="ui-new-table">
								...
								<xsl:with-param name="configUrl">
									<xsl:value-of select="concat($lang-prefix, '/admin/emarket/flushOrderListConfig/', $customer-id, '.json')" />
								</xsl:with-param>
								<xsl:with-param name="toolbarFunction">{}</xsl:with-param>
	...
	<!-- Шаблон значения поля "Юридические лица" -->
	<xsl:template match="property[@name = 'legal_persons']" mode="value">
		<xsl:for-each select = ".//item[not(position() = last())]">
			<xsl:apply-templates select="." mode="value"/>
			<xsl:text>&#44;&#160;</xsl:text>
		</xsl:for-each>
		<xsl:apply-templates select=".//item[position() = last()]" mode="value"/>
	</xsl:template>
	...
	<xsl:template match="group[@name = 'order_delivery_props']" mode="order_delivery_apiship">
		<div class="col-md-12" id="order_delivery_apiship">
			...
			<xsl:variable name="yandexMapApiKey" select="document(concat('uobject://',field[@name='delivery_id']/values/item[1]/@id))/udata/object/properties/group[@name='settings']/property[@name='yandex_map_api_key']/value"/>
			<script src="https://api-maps.yandex.ru/2.1/?lang=ru_RU&amp;apikey={$yandexMapApiKey}" type="text/javascript"/>
	...
	<xsl:template match="field[@name='delivery_point_in']" mode="order_delivery_apiship_field">
		...
		<div class="col-md-6 default-empty-validation">
			...
			<span>
				...
				<script src="/styles/common/js/jquery.mCustomScrollbar.js?{$system-build}"/>
				<script src="/styles/common/js/widget.Delivery.js?{$system-build}"/>
