	...
	<xsl:template match="/result[@method = 'rss_list']/data[@type = 'list' and @action = 'modify']">
		...
		<script><![CDATA[
			$('form').submit(function(event) {
				let rssRaws = $('form').find('tbody tr');
				let emptyFields = [];

				rssRaws.each(function() {
					let rssRow = $(this);
					let fieldsPair = rssRow.find('input.default');
					let nameField = fieldsPair[0];
					let urlField = fieldsPair[1];

					let nameValue = nameField.value;
					let urlValue = urlField.value;

					let nameFilledIn = (nameValue !== '');
					let urlFilledIn = (urlValue !== '');

					let rssAlreadyExists = nameField.getAttribute('name').match(/\[(.*)\]\[/)[1] !== 'new';

					if (nameFilledIn || urlFilledIn || rssAlreadyExists) {
						if (nameValue.trim() === '') {
							emptyFields.push(nameField);
						}

						if (urlValue.trim() === '') {
							emptyFields.push(urlField);
						}
					}
				...
				$(emptyFields).each(function() {
					$(this).effect("highlight", {color: "#00a0dc"}, 2000);
				});
			});
		]]></script>
	</xsl:template>
