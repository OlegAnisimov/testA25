...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<!-- Шаблон вкладки "sitemap" -->
	<xsl:template match="/result[@module = 'seo' and @method = 'sitemap']/data[@type = 'list' and @action = 'view']">
		<div class="tabs-content module">
			<div class="section selected">
				<div class="location">
					<xsl:call-template name="entities.help.button"/>
					...
					<div class="column">
						<xsl:call-template name="ui-new-table">
							...
							<xsl:with-param name="configUrl">
								<xsl:value-of select="concat($lang-prefix, '/admin/seo/flushSitemapConfig/.json')" />
							</xsl:with-param>
							<xsl:with-param name="toolbarFunction">SeoModule.getSiteMapToolBarFunctions()</xsl:with-param>
							<xsl:with-param name="toolbarMenu">SeoModule.getSiteMapToolBarMenu()</xsl:with-param>
						</xsl:call-template>
					</div>
					<div class="column">
						<xsl:call-template name="entities.help.content"/>
	...
	<!-- Шаблон вкладки "robots" -->
	<xsl:template match="/result[@module = 'seo' and @method = 'robots']/data[@type = 'list' and @action = 'view']">
		<div class="tabs-content module">
			<div class="section selected">
				<div class="location">
					<xsl:call-template name="entities.help.button"/>
				...
				<div class="layout">
					<div class="column">
						<xsl:call-template name="ui-new-table">
							<xsl:with-param name="controlParam"><xsl:value-of select="@method"/></xsl:with-param>
							<xsl:with-param name="configUrl">
								<xsl:value-of select="concat($lang-prefix, '/admin/seo/flushRobotsConfig/.json')" />
							</xsl:with-param>
							<xsl:with-param name="toolbarFunction">SeoModule.getRobotsToolBarFunctions()</xsl:with-param>
							<xsl:with-param name="toolbarMenu">SeoModule.getRobotsToolBarMenu()</xsl:with-param>
						</xsl:call-template>
					</div>
					<div class="column">
						<xsl:call-template name="entities.help.content"/>
