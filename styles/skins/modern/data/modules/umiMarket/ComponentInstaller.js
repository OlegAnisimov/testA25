...
function ComponentInstaller() {
	...
	this.showSolutionList = function(params) {
		var content = '<iframe id="solution-list" scrolling="no" src="/admin/umiMarket/getFullSolutionList/" width="900px" height="665px"/>';
		var that = this;
	...
	this.startInstall = function(params) {
		var that = this;
		var messageKey = that.isLastVersion() ? 'js-label-create-backup-warning' : 'js-label-not-last-version-warning-text';
	...
	this.sendRequest = function(step, params, callback) {
		...
		$.post(this.installerUri, request, function(response) {
			...
			that.endInstallWithError(getLabel('js-label-installation-unavailable'), step, params);
		});
	};
	...
	this.handleResponse = function(step, params, response) {
		...
		if (isError) {
			return this.endInstallWithError($('error', response).attr('message'), step, params);
		}
	 ...
	 * @param {String} step название шага на котором произошла ошибка
	 * @param {Object} params параметры с которыми произошла ошибка
	 */
	this.endInstallWithError = function(error, step, params) {
		var that = this;
		that.clearTimeout();
		that.lock = false;

		$.get('/styles/skins/modern/data/modules/umiMarket/html/InstallErrorMessage.html', function(response) {
			that.pushToDialog(response);

			var template = _.template($('#error-message').html());
			var content = template({
				message: error
			});
			that.pushToDialog(content);

			$('a.close-dialog').on('click', function() {
				closeDialog(that.dialogName);
			});

			$('a.retry-button').on('click', function() {
				that.sendRequest(step, params);
			});
		});

		that.sendRequest('cleanup', {}, function() {
			that.sendRequest('restore-supervisor', {}, function() {});
		});
	};
