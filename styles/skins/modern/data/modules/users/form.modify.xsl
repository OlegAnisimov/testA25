...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:umi="http://www.umi-cms.ru/TR/umi">
	...
	<xsl:template match="object" mode="form-modify" priority="1">
		<xsl:variable name="type-guid" select="/result/data/object/@type-guid" />
		<xsl:variable name="show-name">
			<xsl:choose>
				<xsl:when test="($type-guid = 'users-users') or ($type-guid = 'users-author')">
					<xsl:text>1</xsl:text>
				</xsl:when>
				<xsl:otherwise>
					<xsl:text>0</xsl:text>
		...
		<xsl:choose>
			<xsl:when test="$type-guid = 'users-author'" />
		...
		<div class="col-md-10 password-repeat">
			...
			<span>
				<input type="password" name="data[{../../../@id}][password_repeat][]" class="password default"
					   data-password-input-name="{@input_name}" data-login-input-name="data[{../../../@id}][login]"/>
			</span>
		</div>
	</xsl:template>
		...
		<div class="col-md-10 default-empty-validation password-repeat">
			...
			<span>
				<input type="password" name="data[new][password_repeat][]" class="password default"
					   data-password-input-name="{@input_name}" data-login-input-name="data[new][login]"/>
			</span>
		</div>
	</xsl:template>
		...
		<div name="g_{@name}">
			...
			<div class="content">
				...
				<div class="layout">
					<div class="column">
						<div class="row">
							<xsl:if test="$modules-menu/items/item[@name = 'emarket']" >
	...
	<xsl:template match="module">
		<tr>
			<th>
				<i class="big-ico" style="background-image: url('{document(concat('udata://', @name, '/getIconPath/'))/udata}');"></i>
			</th>
			<td>
