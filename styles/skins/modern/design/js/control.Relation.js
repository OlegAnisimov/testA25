...
var ControlRelation = function(options) {
	...
	function loadItems() {
		...
		loadItemsAll(function(response) {
			...
			if (items.length) {
				...
				if (items.length > 0) {
					result = String(items[0].getAttribute('result')).toLowerCase().replace(/[\s]/g, '');
				}
	 ...
	 * @public
	 */
	function loadItemsAll(callback, override, getParam) {
		getParam = getParam || {};
		getParam = $.param(getParam);
		$.ajax({
			url: _sourceUri + _typeId + ".xml?allow-empty" + (getParam ? '&' + getParam : ''),
			type: "get",
			complete: function(r) {
				if (override) {
					callback(r);
	...
	function updateElements(response, callback) {
		...
		for (let i = 0, count = items.length; i < count; i++) {
			...
			for (let j = 0, oldCount = oldValue.length; j < oldCount; j++) {
				if (oldValue[j] == name) {
					oldValue[j] = id;
				}
			}
