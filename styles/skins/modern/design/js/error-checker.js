	...
	var allowedCheckingFunctions = {
		...
		checkEmpty: function(form, $inputContainers) {
			...
			if ($nameInput.length === 0) {
				$nameInput = form.find('input[name="data[name]"]')
			}
			...
			function checkEmptyName($nameInput) {
				let value = $nameInput.val();
				let fieldName = $nameInput.parent().find('acronym').text();

				if (value.trim() === '') {
					elementsWithErrors.push($nameInput[0]);
					errors.push({
			...
			function checkEmptyDefault($defaultDivs) {
				$defaultDivs.each(function() {
					...
					$div.find(inputSelector).each(function() {
						...
						if (($div.hasClass('img_file') || $div.hasClass('file')) && !$(this).attr('id')) {
							return false;
						}
		...
		checkPasswordRepeat: function(form, fields) {
			...
			fields.each(function(){
				...
				var $loginInput = $('input[name="' + $repeatPasswordInput.data('login-input-name') + '"]');
				...
				if ($passwordInput.val() === $loginInput.val()) {
					elementsWithErrors.push($passwordInput[0]);
					elementsWithErrors.push($loginInput[0]);
					errors.push({
						title: getLabel('js-error-password-login-match')
					});
				}
			});
		}
