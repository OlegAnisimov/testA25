$(function() {
	...
	/** Обработчик нажатия на кнопку "Доступно обновление" */
	$('#autoupdateButton').on('click', function(e) {
		e.preventDefault();
		var $button = $(this);

		$.get('/styles/skins/modern/design/js/common/html/updatePopup.html', function(html) {
			openUpdateWindow(html);
		});

		var openUpdateWindow = function(html) {
			openDialog('',  getLabel('js-update-available'), {
				stdButtons: false,
				html: html,
				width: 350,
				openCallback: function() {
					$('#update-text').html(getLabel(
						'js-update-available-text',
						$button.data('version') + '_' + $button.data('revision')
					));
					$('#redirect-to-autoupdate-btn').text(getLabel('js-update-system'));
				}
			});
		};
	});
	...
	$('div.file').each(function() {
		new ControlFile({container: $(this)});
	});
	...
	/** Удаление и управление активностью через форму редактирования объекта или страницы **/
	$(document).ready(function() {
		if (!window.is_page && !window.is_object) {
			return false;
		}

		var DataSet = new dataSet(window.curent_module, true);
		var control = new Control(DataSet, NullItem, {draw: false});
		var recursiveOperator = new PageTreeRecursiveOperator(control);

		DataSet.addEventHandler('onAfterExecute', function(request) {
			if (request.method === 'tree_delete_element' && request.params.is_last_iteration) {
				window.location = window.pre_lang + '/admin/' + window.curent_module + '/';
			}
		});

		DataSet.addEventHandler('onInitComplete', function() {
			/** Переключение активности в форме редактирования страницы */
			$('#is-active-control').click(function() {
				var $button = $('i.small-ico', $(this));
				var checked = $button.hasClass('i-vision');
				recursiveOperator.switchActivity(window.page_id, checked);
				var isActive = false;
				var buttonTitle = getLabel('js-tip-is-noactive');

				if (checked) {
					$button.removeClass('i-vision').addClass('i-hidden');
					isActive = true;
					buttonTitle = getLabel('js-tip-is-active');
				} else {
					$button.removeClass('i-hidden').addClass('i-vision');
				}

				$('#is-active').val(isActive ? 1 : 0);
				$(this).prop('title', buttonTitle);
			});

			/** Удаление в формах редактирования страницы или объекта */
			$('#remove-object').on('click', function () {
				if (window.is_page) {
					var showConfirmation = true;
					recursiveOperator.putToTrash(window.page_id, showConfirmation);
					return;
				}

				// старая логика для удаления объектов
				DataSet.addEventHandler('onBeforeExecute', createConfirm(DataSet));
				DataSet.execute('tree_delete_element', {
					'element': [window.object_id],
					'selected_items': {},
	...
	/** Кнопка "Поделиться" в форме редактирования страницы */
	$('#share-toggler').click(function() {
		$("#ya_share1").toggle('display');
	});

	/** Ининиализация переключения вкладок" */
	$('.market div.tab_button').on('click', function() {
		var $activeTab = $(this).parent();
		$activeTab.hasClass('active') ? $activeTab.removeClass('active') : $activeTab.addClass('active');
	});
});
...
function initSelectizer() {
	$('.default.newselect').each(function() {
		...
		var options = {
			...
			placeholder: $this.attr('umi:placeholder') ? $this.attr('umi:placeholder') : '',
			create: false
		};

		if ($this.hasClass('template_fields')) {
			var insertFieldToTemplate = function() {
				var field = $(this).data('value');
				var templateName = $this.attr('umi:template-name');
				var $template = $('input[name = "' + templateName + '"]');
				var caretPosition = $template.get(0).selectionStart;
				$template.val(insertSubString(field, $template.val(), caretPosition));
				$template.focus();
			};

			options = {
				onFocus: function() {
					$('div.item', this.$control).on('click', insertFieldToTemplate);
				},
				onBlur: function() {
					$('div.item', this.$control).off('click', insertFieldToTemplate);
				}
			};
		}
...
function initOptionedFields() {
	...
	jQuery("div.optioned").each(function() {
		...
		$(addOptionButtonSelector, $container).on('click', function() {
			...
			var optionValue = $(this).parent().parent().find('.edit').val();

			if (_.isEmpty(chosenRelationId) || _.isEmpty(optionValue)) {
				jQuery.jGrowl(getLabel('js-error-add-empty-option-text'), {
					'header': getLabel('js-error-add-empty-option-header')
				});
				return;
			}

			if ($container.attr('umi:unique-relations') && isAlreadyChosenRelation(chosenRelationId, $container)) {
				jQuery.jGrowl(getLabel('js-error-add-same-relation-option-text'), {
					'header': getLabel('js-error-add-empty-option-header')
				});
				return;
			}

			if ($container.attr('umi:unique-value') && isAlreadyAddedOption(optionValue, $container)) {
				jQuery.jGrowl(getLabel('js-error-add-same-value-option-text'), {
					'header': getLabel('js-error-add-empty-option-header')
				});
				return;
			}
			...
			var type = $container.attr('umi:type');
			var disabled = $container.attr('umi:not-editable');
	...
	/**
	 * Определяет была уже создана опция, связанная с заданным элементом справочника
	 * @param {String} id идентификатор элемента справочника
	 * @param {jQuery|Element} $container элемент-контейнер для поля
	 * @returns {boolean}
	 */
	function isAlreadyChosenRelation(id, $container) {
		var alreadyChosenIdList = [];

		$('.values .col-md-6 input', $container).each(function() {
			var alreadyChosenId = $(this).attr('value');
			alreadyChosenId = (Number.isNaN(parseInt(alreadyChosenId))) ? alreadyChosenId : parseInt(alreadyChosenId);
			alreadyChosenIdList.push(alreadyChosenId);
		});

		id = (Number.isNaN(parseInt(id))) ? id : parseInt(id);
		return alreadyChosenIdList.indexOf(id) !== -1;
	}

	/**
	 * Определяет была уже создана опция с заданным значением
	 * @param {String} value проверяемое значение опции
	 * @param {jQuery|Element} $container элемент-контейнер для поля
	 * @returns {boolean}
	 */
	function isAlreadyAddedOption(value, $container) {
		var alreadyAddedValueList = [];

		$('.values .col-md-5 input', $container).each(function() {
			var alreadyAddedValue = $(this).attr('value');
			alreadyAddedValueList.push(alreadyAddedValue);
		});

		return alreadyAddedValueList.indexOf(value) !== -1;
	}
...
$(document).ready(function() {
	$('.selectize-control').on('click', function() {
		if ($(this).find('.selectize-dropdown-content > div').length > 20) {
			$(this).closest('tr').addClass('big_list');
		}
	})
});