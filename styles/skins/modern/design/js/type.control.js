...
var modernTypeController = (function(jQuery) {
	...
	var initWysiwyg = function() {
		if (!isWysiwygLoaded) {
			uAdmin.reg = {};
			$('head').append('<script type="text/javascript" src="/styles/common/js/cms/wysiwyg/wysiwyg.js" />');
			uAdmin('settings', {
				min_height: 100,
		...
		$.post(
			...
			function(data) {
				if (data.data && data.data.error === undefined) {
					...
					showMessage(
						getLabel('js-error-occurred') + ' <br />"' + getErrorMessage(data) + '".',
						getLabel('js-error-header')
					);
				}
			},
			"json"
		...
		$.post(
			...
			function(data) {
				if (data.data && data.data.error === undefined) {
					...
					var errorText = getLabel('js-error-message') + ' "' + getErrorMessage(data) + '".';
	...
	var getErrorMessage = function(data) {
		return (data.data && data.data.error) ? data.data.error : getLabel('js-dispatch-unknown-error');
	};
