...
<xsl:stylesheet version="1.0"
	...
	<xsl:template name="ui-new-table">
        ...
        <xsl:param name="dragAllowed">0</xsl:param>
        ...
        <xsl:variable name="prefix" select="concat($lang-prefix, '/admin/', $module)" />
	...
	<xsl:template name="ui-smc-table">
		...
		<!-- Отображать в корне контрола страницы, если они имеют родительские страницы других иерархических типов -->
		<xsl:param name="ignore-hierarchy-parent-types">0</xsl:param>
		...
		<script type="text/javascript">
			...
			<xsl:if test="$ignore-hierarchy-parent-types = 1">
				defaultRootMode: true,
			</xsl:if>
		...
		<script id="fast-edit-image-preview" type="text/template">
			<![CDATA[
				...
				<% } else { %>
					<img alt="" style="width:13px;height:13px;cursor: pointer;" src="/styles/skins/modern/design/img/image.png"
	...
	<xsl:template name="ui-smc-tree">
		...
		<script type="text/javascript">
			...
			<xsl:variable name="tree-control-id" select="concat('tree-', $module, '-', method, '-', $control-id)"/>

			var oTree = new Control(oDataSet, TreeItem, {
			id : '<xsl:value-of select="$tree-control-id"/>',
			...
			var searchStorage = new SearchAllTextStorage('<xsl:value-of select="$tree-control-id"/>');
			var oFilterController = new filterController(oTree, 'root-pages-type', true, null, {nativeAdvancedMode:true}, searchStorage);
		...
		<div id="buttons_wr" class="col-md-12">
			<div class="btn-select color-blue pull-right" style="width:200px; margin-top:10px;">
				...
				<ul class="list">
					...
					<xsl:choose>
						<xsl:when test="not($result-data/page) or $disable.view.button = 1" />
						<xsl:otherwise>
							<li>
	...
	<xsl:template name="std-form-name">
		...
		<div class="col-md-6">
			...
			<input class="default" maxlength="255" type="text" name="name" value="{$value}" />
		</div>
	</xsl:template>
	...
	<xsl:template name="std-form-alt-name">
		...
		<div class="col-md-6">
			...
			<span>
				<input class="default" maxlength="255" type="text" name="alt-name" value="{$value}" />
			</span>
		</div>
	</xsl:template>
		...
		<div>
			...
			<div class="values">
				<xsl:apply-templates mode="field-optioned">
					...
					<xsl:with-param name="not-editable" select="$not-editable" />
				</xsl:apply-templates>
			</div>

			<div class="layout-row-icon row">
				<div class="col-md-6">
					<xsl:apply-templates select="." mode="optioned.new.option.relation"/>
				</div>
				<div class="col-md-5">
					<div class="layout-col-control">
						<xsl:apply-templates select="." mode="optioned.new.option.input">
	...
	<!-- Шаблон поля для выбора связи с элементом справочника новой опции -->
	<xsl:template match="field" mode="optioned.new.option.relation">
		<xsl:variable name="isPublic" select="./@public-guide = 1" />

		<div>
			<xsl:attribute name="class">
				<xsl:choose>
					<xsl:when test="$isPublic">
						<xsl:text>optioned-select-container-public</xsl:text>
					</xsl:when>
					<xsl:otherwise>
						<xsl:text>optioned-select-container</xsl:text>
					</xsl:otherwise>
				</xsl:choose>
			</xsl:attribute>

			<div class="selectize-container">
				<select umi:guide="{./@guide-id}" umi:name="{./@input_name}"/>
			</div>
		...
		<xsl:if test="$isPublic">
			<div class="layout-col-icon optioned-add-relation-container">
				<a class="icon-action relation-add">
					<i class="small-ico i-add" title="&js-add-relation-item;"/>
				</a>
			</div>
		</xsl:if>
	</xsl:template>

	<!-- Шаблон поля для ввода значения новой опции -->
	<xsl:template match="field" mode="optioned.new.option.input">
		<xsl:param name="type"/>
		<xsl:choose>
			<xsl:when test="$type = 'varchar'">
				<input type="text" class="default edit string" umi:type="{$type}" step="any" autocomplete="off" />
			</xsl:when>
			<xsl:otherwise>
	...
	<xsl:template match="field/values/value" mode="field-optioned">
		...
		<div class="layout-row-icon row">
			<div class="col-md-6" style="min-height:10px;">
				<input type="text" class="default" value="{object/@name}" readonly="readonly" />
