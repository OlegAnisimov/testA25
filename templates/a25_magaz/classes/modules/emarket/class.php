	...
	class emarket_custom extends def_module {
            ...
            public function pay_from_lk(){
                ...
                if ($payment instanceof payment) {
                    session_start();
                    $_SESSION['lk_ord'] = $order_id;
                    $_SESSION['lk_pay'] = $payment->getCodeName();
...
//                $order = $order_b->getObject();
        ...
        public function receipt_link($order_id){
                ...
                if($class_name == 'receipt'){
                  ...
                  $url = "/emarket/get_receipt/{$order_id}/{$getcode}/";
                  return "<a href=\"{$url}\" target=\"_blank\" class=\"button-normal light-color middle-gradient dark-gradient-hover margin-bottom\">Платежная квитанция</a>";
                }else{
                  return "";
                }
        }
    ...
    //                            file_put_contents($_SERVER['DOCUMENT_ROOT']."/my_test_file.txt",print_r($users,true),FILE_APPEND);
    //				throw new publicException("Access denied");
    //			}
