	...
	var module = {
		...
		draw: {
			...
			personal: function (data) {
				...
				var adminPersonal = {
					photo: '/templates/a25_magaz/images/appointments/person.png',
				...
				for (var i in data) {
					var item = person_item.clone(),
						img = $("<img/>", {
							'src': (data[i].photo) ? data[i].photo : '/templates/a25_magaz/images/appointments/person.png'
						});
		...
		binder: {
			calendar: {
				showFreeTime: function (date) {
					...
					if (!date_entries && _self.constants.__choises['personal']['id'] == "*") {
					...
					for (var i in time_range) {
						...
						div_item.click(function () {
							...
							_self.constants.__choises['date'] = selected_date;
							...
							entryStep.find(".online-entry__choise").text(selected_date + " " + $(this).data('time'));
