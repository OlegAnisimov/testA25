...
<xsl:stylesheet	version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="total[. &gt; ../per_page]" mode="np-news-list-pagi">
        <xsl:apply-templates select="document(concat('udata://system/numpages/', ., '/', ../per_page))" mode="np-news-list-pagi" />
    </xsl:template>
    ...
    <xsl:template match="tonext_link" mode="np-news-list-pagi">
        <div class="grid-100 clear-before news-pagi">
            <a class="button-block middle-color dark-hover light-bg middle-border ajax-news-load-more" href="{.}&amp;subj={$subj}" data-page="{@page-num}" data-id="{$document-page-id}">
