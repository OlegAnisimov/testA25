...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="status_notification_receipt">
		...
		<a href="https://{domain}/emarket/receipt/{order_id}/{receipt_signature}/">
	...
	<xsl:template match="invoice_content">
		...
		<p>
			<a href="https://{domain}{invoice_link}">
				<xsl:value-of select="concat('https://', domain, invoice_link)" />
			</a>
		</p>
	</xsl:template>
		...
		<xsl:attribute name="href">
			<xsl:choose>
				<xsl:when test="personal_params">
					<xsl:value-of select="concat('https://', domain, '/emarket/personal/void/', personal_params, '/')" />
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of select="concat('https://', domain, '/emarket/personal/')" />
				</xsl:otherwise>
			</xsl:choose>
		</xsl:attribute>
	</xsl:template>
