...
<xsl:stylesheet	version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="field" mode="legal_form">
        ...
        <div class="form-input">
            <label for="field-{@name}" class="middle-color">
                <xsl:value-of select="$form_title" />
                <span class="active-color">*</span>
            </label>
            <xsl:apply-templates select="." mode="data_form_new">
                <xsl:with-param name="customer_email" select="$customer_email" />
            </xsl:apply-templates>
        </div>
    </xsl:template>

    <xsl:template match="field[@type = 'string']" mode="data_form_new">
        <xsl:param name="customer_email" />
        <input type="text" class="text-input dark-color light-bg" name="{@input_name}" id="field-{@name}" value="{.}" required="required">
